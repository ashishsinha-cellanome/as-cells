...
def plot_statistics(df_stats, output_dir):
    ...
    ax = sns.barplot(x='Class', y='Mean', hue='Split', data=df_stats, palette='colorblind')
    ...
    plt.figure(figsize=(12, 8))
    ax = sns.barplot(x='Class', y='Median', hue='Split', data=df_stats, palette='colorblind')
...
def print_table(df_stats):
    ...
    headers = ["Split", "Class", "Mean", "Median", "Max", "Images With BBoxes", "Total Images"]
    
    widths = [len(h) for h in headers]
    for _, row in df_stats.iterrows():
        row_vals = [
            row['Split'], row['Class'], 
            f"{row['Mean']:.2f}", f"{row['Median']:.1f}", str(row['Max']),
            str(row['ImagesWithBBoxes']), str(row['TotalImages'])
        ]
    ...
    for _, row in df_stats.iterrows():
        row_vals = [
            row['Split'].capitalize(), row['Class'], 
            f"{row['Mean']:.2f}", f"{row['Median']:.1f}", str(int(row['Max'])),
            str(int(row['ImagesWithBBoxes'])), str(int(row['TotalImages']))
        ]
...
def main(cfg: DictConfig):
    ...
    for split_key, split_name in splits.items():
        ...
        for cat_name, coco_cat_id in cat_name_to_id.items():
            ...
            if is_debug and candidate_images:
                ...
                sample_img_id, sample_ann_ids, sample_count = random.choice(candidate_images)
    ...
    if is_debug:
        ...
        plot_debug_grid(debug_samples, str(debug_grid_path), classes_sorted, split_keys_processed)
        
        # 4. Plot Max BBox Grid
        debug_max_grid_path = output_dir / "debug_max_bbox_samples.png"
        plot_debug_grid(max_debug_samples, str(debug_max_grid_path), classes_sorted, split_keys_processed)
