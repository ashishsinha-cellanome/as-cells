...
def main(SAVE_PATH):
    ...
    for TEST_DATASET in TEST_DATASETS:
        ...
        for model_key in MODELS:
            ...
            print(f"Class ID to class name mapping for {model_key}: {model.get_label_map()}")
            ...
            for k, v in model.get_label_map().items():
                if k not in CLASS_MAPPING or v != CLASS_MAPPING[k]:
                    print(f"[WARN] (class_id, class_name) pair ({k}, {v}) is defined in the {model_key} label map but not covered in CLASS_IDS_TO_CLASS_NAMES_MAP")

            # Define annotation filter based on model's detected classes
            class_ids_of_interest = list(model.get_label_map().keys())
            
            if 'cell-adhered' in model.get_class_names() or 'soma' in model.get_class_names():
            ...
            for idx, img_metrics in enumerate(tqdm(eval_results[-1], desc=f"Logging {model_name}/{dataset_name}")):
                ...
                try:
                    ...
                    for class_id in class_ids_of_interest:
                        ...
                        if class_label == 'bg': continue # Don't log 'bg' stats
                        row = {
                            "model": model_key, "dataset": dataset_name, "image": image_name,
                            "class_id": class_id, "class_name": class_label,
                            "tp": img_metrics['tp'][class_id], "fp": img_metrics['fp'][class_id], "fn": img_metrics['fn'][class_id],
                            "precision": img_metrics['precision'][class_id], "recall": img_metrics['recall'][class_id],
                            "f1_score": img_metrics['f1'][class_id], "inference_time_ms": runtimes[idx] * 1000
                        }
                    ...
                    if plot_preds and (idx % 10 == 0): # Only plot every 5th image
                        ...
                        # Plot preds with GT
                        pred_img1 = show_detections_updated(datasample['image'], gt_dets, pred=False, label_map=CLASS_MAPPING) #model.get_label_map()
                        pred_img2 = show_detections_updated(pred_img1, prediction, pred=True, label_map=CLASS_MAPPING)
                        ...
                        save_name_err1 = output_dir / f"errors/{base_img_name}_remap.png"
                    ...
                    print(f"[ERROR] Failed processing image {idx} ({datasample.get('name', 'N/A')}): {e}")
            ...
            for class_id in tqdm(class_ids_of_interest):
                ...
                print (f"[INFO] {model_key.upper()} Precision: {precision_per_class[class_id]:.5f} | Recall: {recall_per_class[class_id]:.5f} | F1: {f1_per_class[class_id]:.5f} at IoU 0.5 for {class_name}")
