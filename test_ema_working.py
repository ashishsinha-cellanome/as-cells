...
def test_ema():
    ...
    for (n1, p1), (n2, p2) in zip(
        model.named_parameters(), ema.module.named_parameters()
    ):
    ...
    print(
        f"   Model-EMA difference: {(linear_weight_model - linear_weight_ema_after).abs().max().item():.2e}"
    )
    ...
    if not torch.equal(bn_mean_before, bn_mean_after):
        ...
        print(
            "⚠️  WARNING: EMA buffers did not change (might be expected if model BN not updating)"
        )
