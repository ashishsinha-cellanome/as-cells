...
def run_overfit_test(model, batch, num_steps=500):
    ...
    labels = [
        {k: v.to(device) for k, v in sample.items()} for sample in batch["labels"]
    ]
    ...
    if len(losses) > 1 and losses[0] > losses[-1]:
        ...
        print(
            f"WARNING: Loss did not decrease significantly. Initial: {losses[0]:.4f}, Final: {losses[-1]:.4f}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Verification script for RT-DETR training setup."
    )
    parser.add_argument(
        "--config",
        "-c",
