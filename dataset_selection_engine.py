...
def extract_features_for_image(img_path, mask_pkl_path, dataset_name, dinov2_model, device):
    ...
    for cat, cat_anns in anns_by_cat.items():
        ...
        for rank, ann in enumerate(top_anns):
            ...
            patches = patch_tokens[0].cpu().numpy() # (256, 768)
            
            # Downsample the perfect binary mask to the 16x16 patch grid
            # Only where crop > 0 (meaning the object is present)
            binary_crop_mask = (resized_crop > 0).any(axis=2).astype(np.uint8)
            patch_mask = cv2.resize(binary_crop_mask, (16, 16), interpolation=cv2.INTER_NEAREST)
            patch_mask_flat = patch_mask.flatten()
            
            foreground_indices = np.where(patch_mask_flat > 0)[0]
            
            # If the object is somehow too small to even register on a 16x16 grid, skip PCA overlay
            if len(foreground_indices) > 0:
                ...
                # Reconstruct full 16x16 grid
                grid_norm_01 = np.zeros((256, 1))
                grid_norm_01[foreground_indices] = fg_pca_norm
                
                grid_255 = (grid_norm_01.reshape(16, 16) * 255).astype(np.uint8)
                ...
                binary_mask_rgb = np.repeat(binary_crop_mask[:, :, np.newaxis], 3, axis=2) * 255
...
def process_all_datasets(model, device):
    ...
    for ds in tqdm(list(base_dir.iterdir())):
        ...
        for img_path, mask_path in valid_imgs:
            class_embs = extract_features_for_image(img_path, mask_path, ds.name, model, device)
...
def generate_visualizations(all_embeddings):
    ...
    for cat, embs in all_embeddings.items():
        ...
        for metric in ['cosine', 'euclidean']:
            ...
            plt.title(f"Pairwise {metric.capitalize()} Distance of DINOv2 Signatures (Class: {class_name})", fontsize=24, pad=20)
...
if __name__ == "__main__":
    ...
    embeddings = process_all_datasets(model, device)
