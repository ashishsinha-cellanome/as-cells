...
def evaluate_cellpose_pr(cellpose_model, dataset, min_iou=0.5):
    ...
    for idx in range(len(dataset)):
        ...
        gt_boxes = annots["annotations"][["xtl", "ytl", "xbr", "ybr"]].values.astype(
            float
        )
    ...
    return num_true_positives / (
        num_true_positives + num_false_positives + 1e-30
    ), num_true_positives / (num_true_positives + num_false_negatives + 1e-30)


def evaluate_yolo_pr(
    predictions, dataset, class_ids_of_interest=None, min_iou=0.5, child_parent_map=None
):
    """
    Args:
    ...
    for idx in range(len(dataset)):
        ...
        if child_parent_map is not None:
            annots = enforce_one_to_one_mapping(
                data_sample=annots, child_parent_map=child_parent_map
            )
...
def evaluate_mask_rcnn_pr(
    predictions,
    dataset,
    class_ids_of_interest=None,
    min_iou=0.5,
...
def calculate_confusion_matrix(
    predictions, dataset, use_mask=False, min_iou=0.5, child_parent_map=None
):
    """
    Args:
        predictions (List of dictionaries): The i-th dictionary in the list is the detection results for
    ...
    for idx in range(len(dataset)):
        ...
        confusion_matrix += calculate_sample_confusion_matrix(
            annots, predictions[idx], gt_class_ids, use_mask, min_iou
        )
...
def calculate_sample_confusion_matrix(
    data_sample, detections, gt_class_ids, use_mask=False, min_iou=0.5
):
