        ...
        for i in range(num_objs):
            ...
            if "masks" in annots and segmentation_model:
                ...
                box_xtl, box_ytl, box_xbr, box_ybr = (
                    annots["annotations"].loc[i, ["xtl", "ytl", "xbr", "ybr"]].values
                )
...
def evaluate_mask_rcnn(mask_rcnn_model, dataset, max_dets=100):
    ...
    for idx in range(len(dataset)):
        ...
        # construct detections in Mask RCNN format
        output = contruct_predictions_from_mask_rcnn(
            preds, annots["image"].shape[1], annots["image"].shape[0]
        )
...
def contruct_predictions_from_mask_rcnn(
    preds: Dict[str, List], image_width: int, image_height: int
) -> dict:
    """
...
def contruct_predictions_from_detections(
