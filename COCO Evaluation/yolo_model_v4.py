...
class Yolov4ObjectDetector:
    def __init__(
        ...
        try:
            ...
            logging.error(
                f"Failed to load class names from the config file. Likely the path to .names file: "
    ...
    def detect(
        ...
        if self._model is None:
            logging.error(
                "YOLOv4 CV model has not been initialized. Please initialize the class before detect()."
            )
            return (
        ...
        if log_time:
            logging.info(f"YOLOv4 object detection took {elap:.4f} seconds")
