...
def save_masks_in_coco_rle_format(out: Dict[str, List], 
                                  image_width: int, 
                                  image_height: int, 
                                  filename: str) -> None:
    """
...
def load_masks_in_coco_rle_format(filename: str, model_label_map: Dict[int, str]) -> \
(Dict[str, np.ndarray], Dict[str, List]):
    ...
    # (while the masks from the instance segmentation model do)
    combined_masks: Dict[str, np.ndarray] = {class_name: np.zeros((image_height, image_width), dtype=np.uint16)
...
def save_combined_masks_in_coco_rle_format(combined_masks: Dict[str, np.ndarray], 
    ...
    reverse_label_map: Dict[str, int] = {value: key for key, value in model_label_map.items()}
    ...
    save_masks_in_coco_rle_format({'boxes': boxes, 'labels': labels, 'masks': masks}, 
                                  image_width, 
                                  image_height, 
                                  filename)
