...
MODEL_WEIGHTS_PATH: Final[str] = (
    "/home/cellareye/Cellanome/dl-mehdi/Semantic Segmentation/checkpoints/bf_fcn_resnet50_1cycle_lrs_4_bs_10_epochs.pt"
)
...
def show_detections(img: np.ndarray, mask: np.ndarray):
    ...
    for class_id in class_ids:
        ...
        image[class_pixels] = (
            0.6 * image[class_pixels] + 0.4 * (mask * COLORS[class_id])[class_pixels]
        )
...
class SemanticSegmentation:
    def __init__(
        ...
        if model_type not in ["FCN", "DLV3"]:
            ...
            logging.warning(
                f"Invalid model type was passed: {model_type}! Trying to use the default FCN model"
            )
            self.model_type = "FCN"
        ...
        if model_type == "DLV3":
            self._model = seg_models.deeplabv3_resnet50(
                weights=seg_models.DeepLabV3_ResNet50_Weights.DEFAULT
            )
        else:
            self._model = seg_models.fcn_resnet50(
                weights=seg_models.FCN_ResNet50_Weights.DEFAULT
            )
        ...
        self._model.classifier[4] = torch.nn.Conv2d(
            num_channels, len(self._label_map) + 1, kernel_size=(1, 1), stride=(1, 1)
        )
    ...
    def detect(
        ...
        scale: float = max(
            larger_side_size / self._max_input_size,
            smaller_side_size / self._min_input_size,
        )
        if scale > 1:
        ...
        if log_time:
            logging.info(
                f"{self.model_type} Semantic Segmentation took {elap:.4f} seconds"
            )
