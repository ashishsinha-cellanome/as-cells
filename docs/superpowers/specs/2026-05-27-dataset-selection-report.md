...
- **Embedding Overlays** (`<dataset-name>_<image_name>_<class_name>_emb.png`): These images overlay a PCA reduction of the DINOv2 spatial patch tokens onto the original cell crop using the `magma` colormap. They visually highlight which specific physical structures (like the nucleus or membrane) the model is focusing on.
...
- **Cluster Visualizations** (`cluster_visualization_class_<class_name>.png` & `_all_classes.png`): 2D PCA scatter plots that map the datasets into physical space, color-coded by their K-Means cluster assignments, with the dataset names clearly annotated using large fonts. Both per-class and 'all classes combined' plots are provided.
