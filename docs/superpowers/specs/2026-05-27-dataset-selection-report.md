...
- **Embedding Overlays** (`<dataset-name>_<image_name>_<class_name>_emb.png`): These images overlay a PCA reduction of the DINOv2 spatial patch tokens onto the original cell crop using the `viridis` colormap. They visually highlight which specific physical structures (like the nucleus or membrane) the model is focusing on. A colorbar (colorscale) is included to map the feature values.
- **Pairwise Distance Heatmaps** (`pairwise_distance_heatmap_class_<class_name>.png` & `_all_classes.png`): High-resolution heatmaps showing the exact mathematical distances between every cell line. The plot dimensions and font sizes dynamically scale to the number of datasets compared to ensure text remains highly legible, with increased font sizes for axes labels and titles.
- **Cluster Visualizations** (`cluster_visualization_class_<class_name>.png` & `_all_classes.png`): 2D t-SNE scatter plots that map the datasets into physical space, color-coded by their K-Means cluster assignments, with the dataset names clearly annotated using large fonts. The figures are specifically formatted as large squares with tight layout parameters ensuring that all points, titles, and legends remain fully contained inside the frame without getting cut off. Both per-class and 'all classes combined' plots are provided.
