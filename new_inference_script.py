...
from models.rt_detr_lightning_module import RTDETRLightningModule, to_cpu_device, convert_to_xywh # Re-import necessary helpers
from data.coco_data_module import COCODataModule
from transformers import RTDetrImageProcessor
from pycocotools.cocoeval import COCOeval

# Import VisionModel and related utilities for cropping
from models.AbstractVisionModel import VisionModel, run_model, post_process_detections, DEFAULT_DETECTION_CONFIDENCE
...
class LightningModelAdapter(VisionModel):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            label_map={int(k): v for k, v in config.model.label_map.items()}, # Ensure label_map is Dict[int, str]
        ...
        if self.use_ema:
            if hasattr(self.pl_module, 'ema_model') and self.pl_module.ema_model:
                ...
                logger.warning("EMA model requested but not found in checkpoint or module. Using regular model.")
        ...
        self._model_input_size = (self.config.data.model_input_size, self.config.data.model_input_size)
    ...
    def detect_batch(
        ...
        for pred in post_processed_outputs:
            boxes = to_cpu_device(pred["boxes"]).numpy() if len(pred["boxes"]) > 0 else np.zeros((0, 4))
            scores = to_cpu_device(pred["scores"]).numpy() if len(pred["scores"]) > 0 else np.zeros((0,))
            labels = to_cpu_device(pred["labels"]).numpy() if len(pred["labels"]) > 0 else np.zeros((0,))
    ...
    def postprocess(self, **kwargs: Any) -> List[Dict[str, list]]:
        ...
        """
        raise NotImplementedError("LightningModelAdapter handles post-processing within detect_batch or delegates to run_model.")
...
def main(config: DictConfig):
    ...
    if 'hyper_parameters' in checkpoint and 'config' in checkpoint['hyper_parameters']:
        ...
        logger.warning("Could not find original training config in checkpoint. Using current config for model setup.")
    ...
    if config.inference.use_ema:
        if 'ema_state_dict' in checkpoint:
            logger.info("EMA model requested and 'ema_state_dict' found in checkpoint. Loading EMA weights.")
            # Create a ModelEma instance and load the state dict
            # We need to initialize ModelEma with a copy of the pl_model.model
            pl_model.ema_model = ModelEma(pl_model.model, decay=0.9999) # Decay rate doesn't matter for inference
            ...
            logger.warning("EMA model requested but 'ema_state_dict' not found in checkpoint. Falling back to regular model.")
    ...
    logger.info(f"Initialized model: {adapter_model.get_model_name()} on device: {adapter_model._device}")
    ...
    if config.inference.input_type == "image":
        ...
        if config.inference.perform_metrics and config.inference.gt_json_path:
            ...
            gt_json_path = Path(hydra.utils.to_absolute_path(config.inference.gt_json_path))
            if gt_json_path.is_file():
                ...
                for coco_img_id in coco_img_ids:
                    ...
                    if img_info['file_name'] == img_filename:
                        images_to_infer.append((img, coco_img_id, str(image_path), coco_gt))
                        logger.info(f"Found GT for image {img_filename} with COCO ID {coco_img_id}")
                        break
                else:
                    logger.warning(f"No COCO GT found for image {img_filename} in {gt_json_path}. Metrics will be skipped for this image.")
                    images_to_infer.append((img, current_image_unique_id, str(image_path), None))
                    current_image_unique_id += 1
            else:
                logger.warning(f"GT JSON not found at {gt_json_path}. Metrics will be skipped.")
                config.inference.perform_metrics = False
                images_to_infer.append((img, current_image_unique_id, str(image_path), None))
                current_image_unique_id += 1
        else:
            images_to_infer.append((img, current_image_unique_id, str(image_path), None)) # Mock image_id
            current_image_unique_id += 1
        ...
        if config.inference.perform_metrics and config.inference.gt_json_path:
            ...
            if gt_json_path.is_file():
                ...
                logger.warning(f"GT JSON not found at {gt_json_path}. Metrics will be skipped for all images in folder.")
                config.inference.perform_metrics = False
        ...
        for img_path in image_files:
            ...
            if coco_gt: # If a GT JSON was loaded for the folder
                ...
                for coco_img_id in coco_img_ids:
                    img_info = coco_gt.loadImgs(coco_img_id)[0]
                    if img_info['file_name'] == img_filename:
                        images_to_infer.append((img, coco_img_id, str(img_path), coco_gt))
                        current_img_coco_gt = coco_gt # Assign the loaded coco_gt
                        break
                else: # No GT found for this specific image
                    images_to_infer.append((img, current_image_unique_id, str(img_path), None))
                    current_image_unique_id += 1
            else: # No GT JSON provided for folder
                images_to_infer.append((img, current_image_unique_id, str(img_path), None)) # Mock image_id
                current_image_unique_id += 1
        ...
        data_module = COCODataModule(
            ...
            org_images_in_model_input_size=config.data.get('org_images_in_model_input_size', False),
        ...
        for i in tqdm(range(len(data_module.test_dataset)), desc="Loading dataset images"):
            coco_img_info = data_module.test_dataset.coco.loadImgs(data_module.test_dataset.ids[i])[0]
            img_path = os.path.join(data_module.test_dataset.root, coco_img_info['file_name'])
            img = Image.open(img_path).convert("RGB")
            images_to_infer.append((img, data_module.test_dataset.ids[i], img_path, test_dataset_coco))
    ...
    for img_idx, (img_pil, image_id, original_path, image_coco_gt_for_this_img) in enumerate(tqdm(images_to_infer, desc="Inferring on images")):
        ...
        detections_raw, runtime_s = run_model(
            ...
            input_image=img_np,
            input_resize=adapter_model.get_cropping_info()[0], # Pass the resize dict from adapter
            input_crop_corners=adapter_model.get_cropping_info()[1], # Pass the crop corners from adapter
        ...
        if config.inference.visualize_preds and img_idx < config.inference.viz_max_samples:
            ...
            logger.info(f"Saved visualization for {Path(original_path).name} to {viz_save_path}")

    # --- 5. Compute Metrics (if enabled) ---
    metrics_output_dir = output_base_dir / dataset_name / adapter_model.get_model_name().replace(" ", "_")
    ...
    if config.inference.perform_metrics and coco_gt:
        ...
        class MockCellMaskDataset:
            def __init__(self, coco_gt_obj, images_data_list):
                ...
                self.images_data = [item for item in images_data_list if item[3] is not None]
                self.ids = [item[1] for item in self.images_data] # COCO image IDs that have GT
                self.image_id_to_original_idx = {item[1]: idx for idx, item in enumerate(images_data_list) if item[3] is not None} # Map COCO ID to its index in the *original* images_to_infer list
                self.class_names_to_ids_map = {cat['name']: cat['id'] for cat in coco_gt_obj.dataset['categories']}
            ...
            def __getitem__(self, idx):
                ...
                return {
                    ...
                    'image': np.array(img_pil), # evaluate_pr_per_image expects np array
        ...
        if not mock_dataset.images_data:
            logger.warning("No images with ground truth annotations were processed. Skipping metrics computation.")
            config.inference.perform_metrics = False
        else:
            ...
            for img_data_from_mock_ds in mock_dataset.images_data:
                ...
                original_idx_in_images_to_infer = image_id_to_original_idx[original_img_id]
                pr_predictions_aligned.append(all_predictions_for_pr_eval[original_idx_in_images_to_infer])
            ...
            (
                ...
                micro_precision, micro_recall, micro_f1_val, 
            ...
            for img_pr_metrics in per_image_pr_metrics:
                row = {
                    ...
                    "image_id": next(item[1] for item in images_to_infer if Path(item[2]).name == img_pr_metrics['name']), # Get actual image_id
                    "inference_time_ms": inference_times_ms[image_id_to_original_idx[next(item[1] for item in images_to_infer if Path(item[2]).name == img_pr_metrics['name'])]]
                }
                for class_id in class_ids_of_interest:
                    class_name = adapter_model.get_label_map().get(class_id, f"ID_{class_id}")
                    ...
                    row[f"{class_name}_Precision"] = img_pr_metrics['precision'][class_id]
            ...
            if per_image_metrics_data_rows:
                ...
                per_image_csv_path = metrics_output_dir / f"{dataset_name}_{adapter_model.get_model_name().replace(' ', '_')}_per_image_metrics.csv"
            ...
            aggregate_json_path = metrics_output_dir / f"{dataset_name}_{adapter_model.get_model_name().replace(' ', '_')}_aggregate_metrics.json"
        ...
        logger.info("Skipping metrics calculation as 'perform_metrics' is False or no ground truth provided.")
