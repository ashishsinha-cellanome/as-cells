...
def get_transform(train: bool = True) -> A.core.composition.Compose:
    ...
    if train:
        ...
        trsfms = [
            A.RandomScale(
                scale_limit=(MIN_RANDOM_SCALE - 1.0, MAX_RANDOM_SCALE - 1.0), p=1.0
            ),
            A.PadIfNeeded(
                min_height=MODEL_INPUT_SIZE,
                min_width=MODEL_INPUT_SIZE,
                position="random",
            ),
            A.RandomCrop(
    ...
    return A.Compose(
        ...
        bbox_params=A.BboxParams(
            format="coco", label_fields=["category"], clip=True, min_area=MIN_AREA
        ),
    )
...
hg_preprocessor: RTDetrImageProcessor = RTDetrImageProcessor.from_pretrained(
...
hg_preprocessor.size = {"height": MODEL_INPUT_SIZE, "width": MODEL_INPUT_SIZE}
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(annotations, dict):
            ...
            boxes: np.ndarray = np.array(
                [record["bbox"] for record in annotations["annotations"]]
            )
            labels: List[int] = [
                record["category_id"] for record in annotations["annotations"]
            ]
            if self.instance_segmentation:
                masks: List[np.ndarray] = [
                    coco_mask_util.decode(record["segmentation"])
        ...
        if self.transforms and len(boxes) > 0:
            if self.instance_segmentation:
                ...
                transformed = self.transforms(
                    image=image_array, bboxes=boxes, masks=masks, category=labels
                )
        ...
        for i, bbox in enumerate(boxes):
            ...
            if self.instance_segmentation:
                record["segmentation"] = coco_mask_util.encode(
                    np.asarray(masks[i], order="F")
                )
                record["segmentation"]["counts"] = record["segmentation"][
                    "counts"
                ].decode("utf8")
...
CROPPED_TRAIN_ANNOTATIONS_PATH = os.path.join(
    DATASET_BASE_PATH, annots_folder_name, "train"
)
CROPPED_TEST_ANNOTATIONS_PATH = os.path.join(
    DATASET_BASE_PATH, annots_folder_name, "test"
)
...
def convert_to_coco_api(
    ...
    id2label: Dict[
        int, str
    ],  # this is for the converted dataset, should start with 0 for YOLO, DETR models (RT/RF),
    ...
    for i, img_filename in enumerate(imgs):
        ...
        if img_name != annots_name:
            print(
                "[ERROR]: Inconsistent annotations file :{} found for image file: {}".format(
                    annots_name, img_name
                )
            )
            return False, {}
    ...
    for idx in tqdm(range(len(imgs))):
        ...
        if instance_segmentation:
            ...
            for record in annots["annotations"]:
                ...
                full_res_mask: np.ndarray = np.zeros(
                    (image_height, image_width), np.uint8
                )
    ...
    json_annotations["categories"] = [
        {"id": i, "name": id2label[i], "supercategory": "biology"}
...
test_dataset_coco = CocoDetection(
    ...
    annFile=os.path.join(
        DATASET_BASE_PATH, "valid_annotations.json"
...
CUSTOM_RT_DETR_WITH_DINOV2_BACKBONE_INITIAL_CHECKPOINT: str = (
    "custom_rt_detr_with_dinov2_backbone"
)
...
if not os.path.exists(os.path.join(MODEL_PATH, DINOV2_BACKBONE_INITIAL_CHECKPOINT)):
    ...
    dinov2_backbone.save_pretrained(
        os.path.join(MODEL_PATH, DINOV2_BACKBONE_INITIAL_CHECKPOINT)
    )
else:
...
if not os.path.exists(
    ...
    print(
        ...
        [
            pretrained_rt_detr_model.model.encoder_input_proj[i][0].weight.shape[1]
            for i in range(len(pretrained_rt_detr_model.model.encoder_input_proj))
        ],
    )
    print(
        "Pretrained config encoder input dims: ",
        pretrained_rt_detr_model.model.backbone.intermediate_channel_sizes,
    )
    ...
    print(
        "DINOv2 model checkpoint of the backbone: ",
        dinov2_backbone_config.dinov2_pretrained_backbone_name_or_path,
    )
    dinov2_backbone = Dinov2BackBoneWithFPN.from_pretrained(
        os.path.join(MODEL_PATH, DINOV2_BACKBONE_INITIAL_CHECKPOINT)
    )
    ...
    rt_detr_model_with_dinov2_backbone_config = RTDetrV2ConfigWithCustomBackBone(
    ...
    test_model = RTDetrV2ForObjectDetectionWithCustomBackbone(
        rt_detr_model_with_dinov2_backbone_config
    )
    print(
        "Reconstructed models encoder input dims: ",
        [
            test_model.model.encoder_input_proj[i][0].weight.shape[1]
            for i in range(len(test_model.model.encoder_input_proj))
        ],
    )
    print(
        "Modified config encoder input dims: ",
        test_model.model.backbone.intermediate_channel_sizes,
    )
    ...
    # save
    pretrained_rt_detr_model.save_pretrained(
        os.path.join(MODEL_PATH, CUSTOM_RT_DETR_WITH_DINOV2_BACKBONE_INITIAL_CHECKPOINT)
    )
    print(
        "backbone_config type for the saved model: ",
        type(pretrained_rt_detr_model.config.backbone_config),
    )
else:
...
def show_sample(idx, dataset):
    ...
    if isinstance(sample, tuple):
        ...
        if (
            ...
            and "segmentation" in annotations["annotations"][0]
        ):
            masks = np.array(
                [
                    coco_mask_util.decode(record["segmentation"])
        ...
        image = (
            (set_mean + sample["pixel_values"].permute(1, 2, 0) * set_var)
            .mul(255)
            .byte()
            .numpy()
            .copy()
    ...
    for i in range(len(labels)):
        ...
        ]  # add 1 to be consistent with Mask R-CNN colors/labels
        if masks is not None:
            color_mask = color * np.repeat(
                np.expand_dims(masks[i][ytl:ybr, xtl:xbr], axis=2), 3, axis=2
            )
...
pre_trained_model_checkpoint: str = os.path.join(
    MODEL_PATH, CUSTOM_RT_DETR_WITH_DINOV2_BACKBONE_INITIAL_CHECKPOINT
)
...
model = RTDetrV2ForObjectDetectionWithCustomBackbone.from_pretrained(
    pretrained_model_name_or_path=pre_trained_model_checkpoint
)
...
class MAPEvaluator:
    ...
    def collect_predictions(self, batch_predictions, batch_image_sizes):
        ...
        post_processed_output = [
            ...
            for outputs in post_processed_output
        ]
    ...
    def __call__(self, evaluation_results, compute_result):
        ...
        post_processed_batch_predictions = self.collect_predictions(
            batch_predictions, batch_image_sizes
        )
        results = {
            int(target["image_id"].item()): output
            for target, output in zip(batch_targets, post_processed_batch_predictions)
        }
        results = convert_preds_to_coco(results)
        self.all_predictions.extend(results)
        self.all_image_ids += [
            int(target["image_id"].item()) for target in batch_targets
        ]
...
eval_compute_metrics_fn = MAPEvaluator(
    ...
    image_processor=hg_preprocessor,
    threshold=0.05,
    max_dets=100,
)
...
if __name__ == "__main__":
    ...
    profiler_handler = torch.profiler.tensorboard_trace_handler(PROFILER_LOG_DIR)
    profiler_config = {
        "activities": [
