...
WEIGHTS_PATH: Final[str] = '/home/cellareye/Cellanome/dl-mehdi/Semantic Segmentation/checkpoints/20241023_sets_1_2_3_6_to_38_0p2_bbox_0p2_b_c_adj_16_bs_10_epochs_1cl_lrs.pt'

def get_sem_seg_model(
    num_classes: int, 
    model_type: str
):
    ...
    if model_type == 'fcn_resnet50':
        ...
        logging.error(f"[ERROR]: Invalid model type: {model_type}! The passed type should be either 'fcn_resnet50' or 'deeplabv3_resnet50'")
        return None
...
class SemanticSegmentator:
    def __init__(
        ...
        self._model: Union[torchvision.models.segmentation.fcn.FCN, torchvision.models.segmentation.deeplabv3.DeepLabV3] = None
        ...
        try:
            ...
            logging.error(
                f"Failed to load instance segmentation model: {repr(ex)}."
            )
        ...
        self._transform = torchvision.transforms.Compose(
            [
                ...
                torchvision.transforms.Resize(size=(self._model_input_size, self._model_input_size)),
    ...
    def predict_batch(
        ...
        adj_crops_list: List[np.ndarray] = [
            cv2.cvtColor(cropped_img, cv2.COLOR_GRAY2BGR) if np.ndim(cropped_img) == 2 else cropped_img 
        ...
        for i in range(0, len(adj_crops_list), batch_size):
            image_tensors: torch.tensor = torch.cat(
                [
                    ...
                    for cropped_img in adj_crops_list[i : min(i + batch_size, len(adj_crops_list))]
                ],
                dim=0,
            )
            ...
            with torch.no_grad():
                ...
                batch_masks = torch.argmax(batch_output, dim=1).cpu().numpy().astype(np.uint8)
        ...
        return [cv2.resize(mask, org_crop_sizes[i], cv2.INTER_NEAREST) for i, mask in enumerate(output_masks)]
    ...
    def predict_cages(
        ...
        if isinstance(cage_bounding_boxes, list):
            ...
            cage_boxes: np.ndarray = np.array(cage_bounding_boxes).copy() if len(cage_bounding_boxes) > 0 else np.zeros((0, 4), dtype=int)
        else:
            cage_boxes: np.ndarray = cage_bounding_boxes.copy()
        # expand the cage bounding boxes by the provided percentage 
        cage_boxes_widths: np.ndarray = (cage_boxes[:, 2] - cage_boxes[:, 0]) * (1 + percentage_to_expand_cage_box_boundaries)
        cage_boxes_heights: np.ndarray = (cage_boxes[:, 3] - cage_boxes[:, 1]) * (1 + percentage_to_expand_cage_box_boundaries)
        ...
        for cage_id in range(cage_boxes.shape[0]):
            ...
            semantic_mask[ytl:ybr, xtl:xbr] = np.maximum(semantic_mask[ytl:ybr, xtl:xbr], masks_list[cage_id])
...
def generate_cage_crops(img: np.ndarray, 
    ...
    for cage_id, (xtl, ytl, xbr, ybr) in enumerate(cage_boxes):
        ...
        cropped_img: np.ndarray = cv2.resize(img[ytl:ybr, xtl:xbr], (crop_size, crop_size), interpolation_scheme)
        # make the crop a 3-channel image (expected by the model) if it is not
        cropped_img = cv2.cvtColor(cropped_img, cv2.COLOR_GRAY2BGR) if np.ndim(cropped_img) == 2 else cropped_img
