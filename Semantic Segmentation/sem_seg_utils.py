...
def show_sample(idx: int, dataset):
    ...
    for class_id in class_ids:
        ...
        image[class_pixels] = (
            0.6 * image[class_pixels]
            + 0.4 / max(class_ids) * (mask * COLORS[class_id])[class_pixels]
        )
...
def pixel_accuracy(
    output: torch.Tensor, mask: torch.Tensor, ignore_index_zero: bool = False
) -> float:
    """
    Pixel-wise accuracy
    ...
    """
    with torch.no_grad():
        predicted_masks: torch.Tensor = torch.argmax(
...
def pixel_accuracy_masks(
    ...
    with torch.no_grad():
        ...
        if ignore_index_zero:
            ...
            accuracy = float(correct[non_zero_idxs].sum()) / float(
                correct[non_zero_idxs].numel()
            )
        else:
...
class SemanticMaskDataset(Dataset):
    def __init__(
        ...
        self.img_names: List[str] = sorted(
            [f for f in self.img_names if f.strip().split(".")[-1] == "jpg"]
        )

        self.mask_names: List[str] = os.listdir(masks_path)
        self.mask_names: List[str] = sorted(
            [f for f in self.mask_names if f.strip().split(".")[-1] == "png"]
        )

        img_file_names: List[str] = [
            ".".join(f.strip().split(".")[:-1]) for f in self.img_names
        ]
        mask_file_names: List[str] = [
            ".".join(f.strip().split(".")[:-1]) for f in self.mask_names
        ]
        ...
        if len(self.mask_names) != len(self.img_names):
            print(
                "[ERROR] The mask filenames and the image filenames are not consistent! Dataset will not be instantiated correctly!"
            )
            return

        if any(
            [
                img_file_names[i] != mask_file_names[i]
                for i in range(min(len(img_file_names), len(mask_file_names)))
            ]
        ):
            print(
                "[ERROR] The mask filenames and the image filenames are not consistent! Dataset will not be instantiated correctly!"
    ...
    def __getitem__(self, idx: int) -> (torch.Tensor, torch.Tensor):
        ...
        semantic_mask: np.ndarray = cv2.imread(
            os.path.join(self.masks_path, mask_name), cv2.IMREAD_UNCHANGED
        )

        # make sure the training image and mask are of the correct size
        image = cv2.resize(
            image, dsize=self.model_input_size, interpolation=cv2.INTER_CUBIC
        )
        semantic_mask = cv2.resize(
            semantic_mask, dsize=self.model_input_size, interpolation=cv2.INTER_NEAREST
        )
...
class DiceCrossEntropyLoss(nn.Module):
    def __init__(
        ...
        self.ce_loss = nn.CrossEntropyLoss(
            weight=ce_classes_weights
        )  # we use ce_classes_weights to give lower weights to bg is needed for CE loss
    ...
    def forward(self, preds, targets):
        ...
        targets_masked[~mask] = (
            0  # needed for one-hot conversion (doesn't matter, will be masked out)
        )

        # One-hot encode targets
        targets_one_hot = F.one_hot(
            targets_masked, num_classes=self.num_classes
        )  # (N, H, W, C)
        ...
        dice_loss = dice_loss.mean() / (
            self.num_classes - (1.0 if self.ignore_index_zero else 0.0)
        )
...
def train(
    ...
    criterion = DiceCrossEntropyLoss(
        num_classes,
        ce_classes_weights=torch.Tensor(
            [1.0 / (K * num_classes - K + 1)]
            + [K * 1.0 / (K * num_classes - K + 1)] * (num_classes - 1)
        ).to(device),
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            iou_score += mIoU(
                output, masks_batch, num_classes=num_classes, ignore_index_zero=True
            )  # ignore the bg in calculating mIoU
            accuracy += pixel_accuracy(
                output, masks_batch, ignore_index_zero=True
            )  # ignore the bg in calculating accuracy
        ...
        torch.save(
            model.state_dict(),
            os.path.join(model_path, "checkpoint_" + str(epoch) + ".pt"),
        )
    ...
    history = {
        ...
        "train_mean_iou": train_ious,
        "test_mean_iou": test_ious,
