...
def distance_batch(in_centers1: np.ndarray, in_centers2: np.ndarray) -> np.ndarray:
    ...
    distances: np.ndarray = np.sqrt(x_distances * x_distances + y_distances * y_distances)  # NxM
    return distances
...
def pair_cages_using_distance(cage_center_coordinates: np.ndarray, reference_cage_center_coordinates: np.ndarray, max_allowed_distance: int = 500):
    # get the distance measure between centers (of the cages)
    distances: np.ndarray = distance_batch(in_centers1=cage_center_coordinates, in_centers2=reference_cage_center_coordinates)
    ...
    unpaired1: List[int] = list(set([i for i in range(cage_center_coordinates.shape[0])]) - set(row_ind))  # type: ignore
    unpaired2: List[int] = list(set([i for i in range(reference_cage_center_coordinates.shape[0])]) - set(col_ind))  # type: ignore
...
def pair_cages_using_iou(cage_boxes: np.ndarray, reference_cage_boxes: np.ndarray, min_iou_for_pairing: float = 0.5):
    # get the distance measure between centers (of the cages)
    ...
    # remove the assignments that are farther than max_allowed_distance
    paired_idx: List[Tuple[int, int]] = []
    unpaired1: List[int] = list(set([i for i in range(cage_boxes.shape[0])]) - set(row_ind))  # type: ignore
    unpaired2: List[int] = list(set([i for i in range(reference_cage_boxes.shape[0])]) - set(col_ind))  # type: ignore
    for (i, j) in list(zip(row_ind, col_ind)):
...
def get_list_of_raw_bf_images(exp_base_path: str, scan_id: int, lane_id: int):
    raw_images_path: str = os.path.join(exp_base_path, 'scan_' + str(scan_id), 'lane_' + str(lane_id))
    raw_images: List[str] = os.listdir(raw_images_path)
    raw_bf_images: List[str] = [img_name for img_name in raw_images if '_White.png' in img_name]
...
def load_masks_in_coco_rle_format(
    ...
    id_boxes: Dict[str, Dict[int, np.ndarray]] = {class_name: {} for class_name in model_label_map.values()}
    ...
    return id_boxes, combined_masks, {"boxes": boxes, "labels": labels, "masks": masks}, model_info
...
def get_fl_channel_filenames(bf_img_name: str, all_img_names: List[str], fl_channel_str_identifiers: List[str]):
    ...
    for img_name in all_img_names:
        ...
        if parsed_img_name[5] == x_coor_str and parsed_img_name[6] == y_coor_str and channel_identifier in fl_channel_str_identifiers:
...
def load_images(
    ...
    out_images: Dict[str, np.ndarray] = {'White':  cv2.imread(os.path.join(raw_images_folder, bf_img_filename), cv2.IMREAD_UNCHANGED)}

    for channel_identifier, fl_img_name in fl_img_filenames.items():
        # load fluorescent channels
        out_images[channel_identifier]: np.ndarray = cv2.imread(os.path.join(raw_images_folder, fl_img_name), cv2.IMREAD_UNCHANGED)
        # apply a median filter to fluorescent channels to fill the holes and smooth the images
        out_images[channel_identifier] = cv2.medianBlur(src=out_images[channel_identifier], ksize=3)
...
def load_images_and_mask_rcnn_results(
    ...
    if not os.path.exists(os.path.join(mask_rcnn_results_folder, results_filename)) or run_the_model:
        ...
        preds, run_time = run_mask_rcnn(
            ...
            post_process_class_names=list(inst_segmentor_model.get_label_map().values()), 
    ...
    _, combined_obj_masks, preds, _ = load_masks_in_coco_rle_format(filename=os.path.join(mask_rcnn_results_folder, results_filename))
...
def get_avg_fl_signal_per_cell_for_fov(
    ...
    fl_channel_identifiers_list: List[str] = [
        channel_identifier for channel_identifier in out_images.keys() if channel_identifier.lower() != 'white'
    ]
    
    total_fl_signal_per_cell: Dict[str, List[int]] = {channel_identifier: [] for channel_identifier in fl_channel_identifiers_list}
    avg_fl_signal_per_cell: Dict[str, List[float]] = {channel_identifier: [] for channel_identifier in fl_channel_identifiers_list}

    for obj_id, box in enumerate(preds['boxes']):
        # only consider detected objects of class cell
        if preds['labels'][obj_id] != inst_segmentor_model.get_reverse_label_map()['cell']:
            continue
        ...
        for channel_identifier in fl_channel_identifiers_list:
            if return_avg_of_log_fl_values:
                fl_img: np.ndarray = convert_to_log_scale(out_images[channel_identifier][ytl:ybr, xtl:xbr])
            else:
                fl_img: np.ndarray = out_images[channel_identifier][ytl:ybr, xtl:xbr].astype(float)
            ...
            # average values
            avg_fl_signal_per_cell[channel_identifier].append(np.sum(masked_fl_img) / cell_area)
...
def get_avg_fl_signal_per_cell_for_scan_and_lane(
    ...
    if return_avg_of_log_fl_values:
        saved_filename_avg: str = 'avg_fl_signal_per_cell_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '_avg_log.csv' 
        saved_filename_total: str = 'total_fl_signal_per_cell_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '_avg_log.csv' 
    else:
        saved_filename_avg: str = 'avg_fl_signal_per_cell_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '.csv' 
        saved_filename_total: str = 'total_fl_signal_per_cell_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '.csv' 
        
    
    if not os.path.exists(os.path.join(exp_base_path, saved_filename_avg)):
        
        total_fl_signal_per_cell = {channel_identifier: [] for channel_identifier in fl_channel_identifiers}
        avg_fl_signal_per_cell = {channel_identifier: [] for channel_identifier in fl_channel_identifiers}

        raw_bf_images: List[str] = get_list_of_raw_bf_images(exp_base_path, scan_id, lane_id)
        raw_images_path: str = os.path.join(exp_base_path, 'scan_' + str(scan_id), 'lane_' + str(lane_id))
        mask_rcnn_results_path: str = os.path.join(exp_base_path, 'scan_' + str(scan_id), 'lane_' + str(lane_id) + '_results')
        ...
        avg_fl_signal_per_cell.to_csv(os.path.join(exp_base_path, saved_filename_avg), index = False)
        
        total_fl_signal_per_cell = pd.DataFrame(total_fl_signal_per_cell)
        total_fl_signal_per_cell.to_csv(os.path.join(exp_base_path, saved_filename_total), index = False)
    else:
        print(f"[INFO] Fluorescent signal stats per cell are already available for scan {scan_id}, lane {lane_id}! loading from file")
...
def get_fl_signal_per_cell_pixel_for_scan_and_lane(
    ...
    # check if the results already exists
    if not os.path.exists(os.path.join(exp_base_path, 'fl_signal_per_cell_pixel_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '.csv')):

        # the histogram of pixel values for each fl channel is returned as a series
        fl_signal_per_cell_pixel = {channel_identifier: pd.Series(data = 0, index=np.arange(0, 2 ** 12)) 
        ...
        for bf_img_filename in raw_bf_images:
            ...
            all_cells_mask: np.ndarray = np.zeros(combined_obj_masks['cell'].shape, np.uint8)
            all_cells_mask[combined_obj_masks['cell'] > 0] = 1
        
            for channel_identifier in fl_channel_identifiers:
                masked_fl_img: np.ndarray = out_images[channel_identifier][all_cells_mask > 0]
                # convert to pandas DataFrame for each of value count calculations
                masked_fl_img_df: pd.DataFrame = pd.DataFrame(data=masked_fl_img.flatten(), columns=['fl_pixel_value'])
                pixel_value_counts: pd.Series = masked_fl_img_df.value_counts(subset='fl_pixel_value')
                
                fl_signal_per_cell_pixel[channel_identifier].loc[pixel_value_counts.index] += pixel_value_counts.values

        # combine all in a DataFrame
        fl_signal_per_cell_pixel = pd.DataFrame(fl_signal_per_cell_pixel)
        # save them to disk
        fl_signal_per_cell_pixel.to_csv(
            os.path.join(exp_base_path, 'fl_signal_per_cell_pixel_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '.csv'), index = False
        )
    else:
        print(f"[INFO] Fluorescent signal stats per cell are already available for scan {scan_id}, lane {lane_id}! loading from file")
        fl_signal_per_cell_pixel =  pd.read_csv(
            os.path.join(exp_base_path, 'fl_signal_per_cell_pixel_scan_' + str(scan_id) + '_lane_' + str(lane_id) +  '.csv')
        )
...
def get_cages_metrics_for_fov(
    ...
    # and load the results to get the combined_mask
    combined_obj_masks, preds = mask_rcnn_results['combined_obj_masks'], mask_rcnn_results['predictions']
    
    cage_boxes: List[np.ndarray] = [box for i, box in enumerate(preds['boxes']) 
                                    if preds['labels'][i] == inst_segmentor_model.get_reverse_label_map()['cage']]

    cage_boxes = np.array(cage_boxes) if len(cage_boxes) > 0 else np.zeros((0, 4), dtype=int)
    ...
    cage_box_idx_to_mask_rcnn_results_idx_map: Dict[int, int] = {
        ...
        (
            [i for i, class_id in enumerate(preds['labels']) if class_id == inst_segmentor_model.get_reverse_label_map()['cage']]
        )
    }
    ...
    if not skip_mask_rcnn_metrics:
        ...
        for metric_channel_identifier in fl_channels_list_for_metric_extraction:
            cage_metrics['all']['total_' + metric_channel_identifier + '_cell']: Dict[int, int] = {}
            cage_metrics['all']['avg_' + metric_channel_identifier + '_cell']: Dict[int, float] = {}
            if metric_channel_identifier in gating_threshold_dict:
                cage_metrics['all']['num_' + metric_channel_identifier + '_positive_cell']: Dict[int, float] = {}
    
    cage_metrics['all']['num_pixels']: Dict[int, int] = {}
    for metric_channel_identifier in fl_channels_list_for_metric_extraction:
        # metrics to be extracted (total and average only defined for now)
        cage_metrics['all']['total_' + metric_channel_identifier + '_pixel']: Dict[int, int] = {}
        cage_metrics['all']['avg_' + metric_channel_identifier + '_pixel']: Dict[int, float] = {}
        if metric_channel_identifier in gating_threshold_dict:
            cage_metrics['all']['num_' + metric_channel_identifier + '_positive_pixel']: Dict[int, float] = {}
    ...
    for gating_channel_identifier in gating_threshold_dict:
        ...
        if not skip_mask_rcnn_metrics:
            cage_metrics[gating_channel_identifier + '_positive']['num_cells']: Dict[int, int] = {}
            cage_metrics[gating_channel_identifier + '_negative']['num_cells']: Dict[int, int] = {}

            cage_metrics[gating_channel_identifier + '_positive']['num_cell_pixels']: Dict[int, int] = {}
            cage_metrics[gating_channel_identifier + '_negative']['num_cell_pixels']: Dict[int, int] = {}
        
            for metric_channel_identifier in fl_channels_list_for_metric_extraction:
                cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_cell']: Dict[int, int] = {}
                cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_cell']: Dict[int, int] = {}
            
                cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_cell']: Dict[int, float] = {}
                cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_cell']: Dict[int, float] = {}
                if metric_channel_identifier in gating_threshold_dict:
        ...
        cage_metrics[gating_channel_identifier + '_positive']['num_pixels']: Dict[int, int] = {}
        cage_metrics[gating_channel_identifier + '_negative']['num_pixels']: Dict[int, int] = {}
        
        for metric_channel_identifier in fl_channels_list_for_metric_extraction:
            cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_pixel']: Dict[int, int] = {}
            cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_pixel']: Dict[int, int] = {}

            cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_pixel']: Dict[int, float] = {}
            cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_pixel']: Dict[int, float] = {}
        ...
        # cage position
        cage_metrics[gating_channel_identifier + '_positive']['fov_filename']: Dict[int, str] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_xtl']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_ytl']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_xbr']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_ybr']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_center_x']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_positive']['cage_center_y']: Dict[int, float] = {}

        cage_metrics[gating_channel_identifier + '_negative']['fov_filename']: Dict[int, str] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_xtl']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_ytl']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_xbr']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_ybr']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_center_x']: Dict[int, float] = {}
        cage_metrics[gating_channel_identifier + '_negative']['cage_center_y']: Dict[int, float] = {}
    ...
    for i, cage_semantic_mask in enumerate(cage_semantic_masks):
        ...
        cage_mask_from_mask_rcnn: np.ndarray = preds['masks'][cage_box_idx_to_mask_rcnn_results_idx_map[i]]

        # number of cell objects detected inside this cage from Mask R-CNN
        caged_cell_instances_mask: np.ndarray = combined_obj_masks['cell'][ytl:ybr, xtl:xbr] * cage_mask_from_mask_rcnn.astype(int)
        cell_ids_in_cage: np.ndarray = np.unique(caged_cell_instances_mask[caged_cell_instances_mask > 0])
        ...
        cage_metrics['all']['cage_center_x'][i + starting_cage_id] = (xtl + xbr) / 2.0 + fov_start_x
        cage_metrics['all']['cage_center_y'][i + starting_cage_id] = (ytl + ybr) / 2.0 + fov_start_y
        
        for gating_channel_identifier in gating_threshold_dict.keys():
            cage_metrics[gating_channel_identifier + '_positive']['fov_filename'][i + starting_cage_id] = bf_img_filename
            cage_metrics[gating_channel_identifier + '_positive']['cage_xtl'][i + starting_cage_id] = xtl + fov_start_x
            cage_metrics[gating_channel_identifier + '_positive']['cage_ytl'][i + starting_cage_id] = ytl + fov_start_y
            cage_metrics[gating_channel_identifier + '_positive']['cage_xbr'][i + starting_cage_id] = xbr + fov_start_x
            cage_metrics[gating_channel_identifier + '_positive']['cage_ybr'][i + starting_cage_id] = ybr + fov_start_y
            cage_metrics[gating_channel_identifier + '_positive']['cage_center_x'][i + starting_cage_id] = (xtl + xbr) / 2.0 + fov_start_x
            cage_metrics[gating_channel_identifier + '_positive']['cage_center_y'][i + starting_cage_id] = (ytl + ybr) / 2.0 + fov_start_y

            cage_metrics[gating_channel_identifier + '_negative']['fov_filename'][i + starting_cage_id] = bf_img_filename
            cage_metrics[gating_channel_identifier + '_negative']['cage_xtl'][i + starting_cage_id] = xtl + fov_start_x
            cage_metrics[gating_channel_identifier + '_negative']['cage_ytl'][i + starting_cage_id] = ytl + fov_start_y
            cage_metrics[gating_channel_identifier + '_negative']['cage_xbr'][i + starting_cage_id] = xbr + fov_start_x
            cage_metrics[gating_channel_identifier + '_negative']['cage_ybr'][i + starting_cage_id] = ybr + fov_start_y
            cage_metrics[gating_channel_identifier + '_negative']['cage_center_x'][i + starting_cage_id] = (xtl + xbr) / 2.0 + fov_start_x
            cage_metrics[gating_channel_identifier + '_negative']['cage_center_y'][i + starting_cage_id] = (ytl + ybr) / 2.0 + fov_start_y
        ...
        for metric_channel_identifier in fl_channels_list_for_metric_extraction:
            ...
            # cropped metric mask
            if return_avg_of_log_fl_values:
                metric_mask: np.ndarray = convert_to_log_scale(fl_img=out_images[metric_channel_identifier][ytl:ybr, xtl:xbr])
            else:
                metric_mask: np.ndarray = out_images[metric_channel_identifier][ytl:ybr, xtl:xbr].astype(float)    
            ...
            if metric_channel_identifier in gating_threshold_dict:
                ...
                metric_positive_mask[metric_mask >= gating_threshold_dict[metric_channel_identifier]] = 1
            ...
            if not skip_mask_rcnn_metrics:
                ...
                foreground_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
                ...
                for cell_id in cell_ids_in_cage:
                    ...
                    foreground_cells_mask_from_mask_rcnn = np.maximum(foreground_cells_mask_from_mask_rcnn, 
                                                                      this_cell_mask * cage_mask_from_mask_rcnn)
                    
                cage_metrics['all']['num_cells'][i + starting_cage_id] = len(cell_ids_in_cage)
                cage_metrics['all']['num_cell_pixels'][i + starting_cage_id] = int(np.sum(foreground_cells_mask_from_mask_rcnn))

                cage_metrics['all']['total_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                    np.sum(metric_mask * foreground_cells_mask_from_mask_rcnn)
                )
                cage_metrics['all']['avg_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                    np.sum(metric_mask * foreground_cells_mask_from_mask_rcnn) / float(np.sum(foreground_cells_mask_from_mask_rcnn)) 
                    if np.sum(foreground_cells_mask_from_mask_rcnn) > 0 else np.nan
                )

                if not return_avg_of_log_fl_values:
                    temp = cage_metrics['all']['total_' + metric_channel_identifier + '_cell'][i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics['all']['total_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = temp

                    temp = cage_metrics['all']['avg_' + metric_channel_identifier + '_cell'][i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics['all']['avg_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = temp

                if metric_channel_identifier in gating_threshold_dict:
                    cage_metrics['all']['num_' + metric_channel_identifier + '_positive_cell'][i + starting_cage_id] = (
                        # the result will be int any way and casting is not needed
                        int(np.sum(metric_positive_mask * foreground_cells_mask_from_mask_rcnn))
                    )
                ...
                for gating_channel_identifier, threshold in gating_threshold_dict.items():
                    ...
                    if return_avg_of_log_fl_values:
                        gating_mask: np.ndarray = convert_to_log_scale(fl_img=out_images[gating_channel_identifier][ytl:ybr, xtl:xbr])
                    else:
                        gating_mask: np.ndarray = out_images[gating_channel_identifier][ytl:ybr, xtl:xbr].astype(float)
                        
                    num_positive_cells: int = 0
                    num_negative_cells: int = 0
                    
                    positive_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
                    negative_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
                    ...
                    for cell_id in cell_ids_in_cage:
                        ...
                        # take the union, we multiply by cage_mask_from_mask_rcnn to only consider parts inside the cage
                        # we do not skip cells that are partially (even very small portion) inside the cage
                        if np.sum(gating_mask * this_cell_mask) >= threshold * np.sum(this_cell_mask):
                            num_positive_cells += 1
                            positive_cells_mask_from_mask_rcnn = np.maximum(positive_cells_mask_from_mask_rcnn, 
                                                                            this_cell_mask * cage_mask_from_mask_rcnn)
                        else:
                            num_negative_cells += 1
                            negative_cells_mask_from_mask_rcnn = np.maximum(negative_cells_mask_from_mask_rcnn, 
                                                                            this_cell_mask * cage_mask_from_mask_rcnn)

                
                    cage_metrics[gating_channel_identifier + '_positive']['num_cells'][i + starting_cage_id] = num_positive_cells
                    cage_metrics[gating_channel_identifier + '_negative']['num_cells'][i + starting_cage_id] = num_negative_cells

                    cage_metrics[gating_channel_identifier + '_positive']['num_cell_pixels'][i + starting_cage_id] = (
                        int(np.sum(positive_cells_mask_from_mask_rcnn))
                    )
                    cage_metrics[gating_channel_identifier + '_negative']['num_cell_pixels'][i + starting_cage_id] = (
                        int(np.sum(negative_cells_mask_from_mask_rcnn))
                    )
                    
                    cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                        np.sum(metric_mask * positive_cells_mask_from_mask_rcnn)
                    )
                    cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                        np.sum(metric_mask * negative_cells_mask_from_mask_rcnn)
                    )
                
                    cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                        np.sum(metric_mask * positive_cells_mask_from_mask_rcnn) / float(np.sum(positive_cells_mask_from_mask_rcnn)) 
                        if np.sum(positive_cells_mask_from_mask_rcnn) > 0 else np.nan
                    )
                    cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_cell'][i + starting_cage_id] = (
                        np.sum(metric_mask * negative_cells_mask_from_mask_rcnn) / float(np.sum(negative_cells_mask_from_mask_rcnn)) 
                        if np.sum(negative_cells_mask_from_mask_rcnn) > 0 else np.nan
                    )

                    if not return_avg_of_log_fl_values:
                        temp = cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id]
                        temp = np.log10(temp) if temp > 0 else np.nan
                        cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id] = temp

                        temp = cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id]
                        temp = np.log10(temp) if temp > 0 else np.nan
                        cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id] = temp

                        temp = cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id]
                        temp = np.log10(temp) if temp > 0 else np.nan
                        cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id] = temp

                        temp = cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id]
                        temp = np.log10(temp) if temp > 0 else np.nan
                        cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_cell'][
                        i + starting_cage_id] = temp
                    ...
                    if metric_channel_identifier in gating_threshold_dict:
                        cage_metrics[gating_channel_identifier + '_positive'][
                        'num_' + metric_channel_identifier + '_positive_cell'
                        ][i + starting_cage_id] = int(np.sum(metric_positive_mask * positive_cells_mask_from_mask_rcnn))
                        cage_metrics[gating_channel_identifier + '_negative'][
                        'num_' + metric_channel_identifier + '_positive_cell'
                        ][i + starting_cage_id] = int(np.sum(metric_positive_mask * negative_cells_mask_from_mask_rcnn))
            ...
            foreground_cells_mask[cage_semantic_mask == sem_segmentor_model.get_reverse_label_map()['cell']] = 1
            ...
            cage_metrics['all']['num_pixels'][i + starting_cage_id] = int(np.sum(foreground_cells_mask))
            
            cage_metrics['all']['total_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                np.sum(metric_mask * foreground_cells_mask)
            )
            cage_metrics['all']['avg_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                np.sum(metric_mask * foreground_cells_mask) / float(np.sum(foreground_cells_mask)) 
                if np.sum(foreground_cells_mask) > 0 else np.nan
            )
            
            if not return_avg_of_log_fl_values:
                temp = cage_metrics['all']['total_' + metric_channel_identifier + '_pixel'][i + starting_cage_id]
                temp = np.log10(temp) if temp > 0 else np.nan
                cage_metrics['all']['total_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = temp

                temp = cage_metrics['all']['avg_' + metric_channel_identifier + '_pixel'][i + starting_cage_id]
                temp = np.log10(temp) if temp > 0 else np.nan
                cage_metrics['all']['avg_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = temp

            
            if metric_channel_identifier in gating_threshold_dict:
                cage_metrics['all']['num_' + metric_channel_identifier + '_positive_pixel'][i + starting_cage_id] = (
                int(np.sum(metric_positive_mask * foreground_cells_mask))
            )
            ...
            for gating_channel_identifier, threshold in gating_threshold_dict.items():
                ...
                positive_pixels_mask: np.ndarray = np.zeros(cage_semantic_mask.shape, np.uint8)
                positive_pixels_mask[gating_mask * foreground_cells_mask >= threshold] = 1
    
                # do the same for effector cells (to find dead effectors)
                negative_pixels_mask: np.ndarray = np.zeros(cage_semantic_mask.shape, np.uint8)
                ...
                cage_metrics[gating_channel_identifier + '_positive']['num_pixels'][i + starting_cage_id] = int(np.sum(positive_pixels_mask))
                cage_metrics[gating_channel_identifier + '_negative']['num_pixels'][i + starting_cage_id] = int(np.sum(negative_pixels_mask))
                
                cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                    np.sum(metric_mask * positive_pixels_mask)
                )
                cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                    np.sum(metric_mask * negative_pixels_mask)
                )
                
                cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                    np.sum(metric_mask * positive_pixels_mask) / float(np.sum(positive_pixels_mask)) 
                    if np.sum(positive_pixels_mask) > 0 else np.nan
                )
                cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_pixel'][i + starting_cage_id] = (
                    np.sum(metric_mask * negative_pixels_mask) / float(np.sum(negative_pixels_mask)) 
                    if np.sum(negative_pixels_mask) > 0 else np.nan
                )

                if not return_avg_of_log_fl_values:
                    temp = cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics[gating_channel_identifier + '_positive']['total_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id] = temp

                    temp = cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics[gating_channel_identifier + '_negative']['total_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id] = temp

                    temp = cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics[gating_channel_identifier + '_positive']['avg_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id] = temp

                    temp = cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id]
                    temp = np.log10(temp) if temp > 0 else np.nan
                    cage_metrics[gating_channel_identifier + '_negative']['avg_' + metric_channel_identifier + '_pixel'][
                    i + starting_cage_id] = temp
                ...
                if metric_channel_identifier in gating_threshold_dict:
                    cage_metrics[gating_channel_identifier + '_positive'][
                    'num_' + metric_channel_identifier + '_positive_pixel'
                    ][i + starting_cage_id] = int(np.sum(metric_positive_mask * positive_pixels_mask))
                
                    cage_metrics[gating_channel_identifier + '_negative'][
                    'num_' + metric_channel_identifier + '_positive_pixel'
                    ][i + starting_cage_id] = int(np.sum(metric_positive_mask * negative_pixels_mask))
                
        
    out: Dict[str, pd.DataFrame] = {}
    out['all'] = pd.DataFrame(cage_metrics['all'])
    for gating_channel_identifier in gating_threshold_dict:
        out[gating_channel_identifier + '_positive'] = pd.DataFrame(cage_metrics[gating_channel_identifier + '_positive'])
        out[gating_channel_identifier + '_negative'] = pd.DataFrame(cage_metrics[gating_channel_identifier + '_negative'])
...
def get_cages_metrics_for_scan_and_lane(
    ...
    base_filename: str = 'cage_metrics_scan_' + str(scan_id) + '_lane_' + str(lane_id) + '_'
    all_filename: str = os.path.join(exp_base_path, base_filename + 'all.csv')
    if return_avg_of_log_fl_values:
        all_filename: str = os.path.join(exp_base_path, base_filename + 'all_avg_log.csv')
    ...
    for gating_id, threshold in log_gating_threshold_dict.items():
        ...
        positive_filenames[gating_id]: str = os.path.join(exp_base_path, base_filename + gating_id + '_positive_' + thr_str + '.csv')
        negative_filenames[gating_id]: str = os.path.join(exp_base_path, base_filename + gating_id + '_negative_' + thr_str + '.csv')
        ...
        for bf_img_name in raw_bf_images:
            ...
            for gating_id in log_gating_threshold_dict:
                cages_metrics_dict[gating_id + '_positive'] = pd.concat(
                    [cages_metrics_dict[gating_id + '_positive'], cages_metrics_fov[gating_id + '_positive']]
                )
                cages_metrics_dict[gating_id + '_negative'] = pd.concat(
                    [cages_metrics_dict[gating_id + '_negative'], cages_metrics_fov[gating_id + '_negative']]
                )

        # save the results to disk
        cages_metrics_dict['all'].to_csv(all_filename, index = False)
        for gating_id in log_gating_threshold_dict: 
            cages_metrics_dict[gating_id + '_positive'].to_csv(positive_filenames[gating_id], index = False)
            cages_metrics_dict[gating_id + '_negative'].to_csv(negative_filenames[gating_id], index = False)
    else:
        print(f"[INFO] Cage metrics are already available for scan {scan_id}, lane {lane_id}! loading from file")

        cages_metrics_dict['all'] = pd.read_csv(all_filename)
        for gating_id in log_gating_threshold_dict:
            cages_metrics_dict[gating_id + '_positive'] = pd.read_csv(positive_filenames[gating_id])
            cages_metrics_dict[gating_id + '_negative'] = pd.read_csv(negative_filenames[gating_id])
...
def prepare_data_for_scan(
    ...
    for lane_id in lane_ids_list:
        ...
        _, avg_fl_signal_per_cell[lane_id] = get_avg_fl_signal_per_cell_for_scan_and_lane(
    ...
    for exp_lane_id in np.unique(list(control_lane_to_experiment_lane_map.values())):
        ...
        for gating_channel_identifier in fl_channels_list_for_gating:
            if gating_channel_identifier in log_gating_threshold_overwrite_dict:
                log_threshold = log_gating_threshold_overwrite_dict[gating_channel_identifier]
            else:
                data = avg_fl_signal_per_cell[exp_lane_id][gating_channel_identifier].values
                ...
                log_threshold = gmm_fit_2(avg_fl_signal_per_cell[exp_lane_id], gating_channel_identifier)
        ...
        log_gating_threshold_dicts_for_exp_lanes[exp_lane_id] = log_gating_threshold_dict
...
def get_cage_crop(
    ...
    (merged_cages_metrics['all']['cage_id'] == cage_id) & (merged_cages_metrics['all']['scan_id'] == scan_id), 
    ['cage_xtl', 'cage_ytl', 'cage_xbr', 'cage_ybr', 'fov_filename', 'fov_filename_ref']
    ]

    if len(cage_location_info) == 0:
        print(f"[ERROR] Cage ID {cage_id} does not exists in the passed merged_cages_metrics! Returning empty ...")
        return np.array([], np.uint8), np.array([], np.uint8), np.array([], np.uint8)
    
    bf_img_name: str = cage_location_info['fov_filename'].iloc[0]
    cage_coords: np.ndarray = cage_location_info[['cage_xtl', 'cage_ytl', 'cage_xbr', 'cage_ybr']].values[0].astype(int)
    ...
    ref_raw_images_path: str = os.path.join(exp_base_path, 'scan_' + str(ref_scan_id), 'lane_' + str(lane_id))
    ref_mask_rcnn_results_path: str = os.path.join(exp_base_path, 'scan_' + str(ref_scan_id), 'lane_' + str(lane_id) + '_results')
    ...
    cage_boxes: np.ndarray = np.array(
        [box for i, box in enumerate(ref_mask_rcnn_results['predictions']['boxes']) if 
         ref_mask_rcnn_results['predictions']['labels'][i] == inst_segmentor_model.get_reverse_label_map()['cage']]
    )

    cage_idxs: np.ndarray = [i for i, label in enumerate(ref_mask_rcnn_results['predictions']['labels']) if 
                             label == inst_segmentor_model.get_reverse_label_map()['cage']]


    iou_matrix: np.ndarray = iou_batch(bboxes1=np.array([[xc_tl, yc_tl, xc_br, yc_br]]), bboxes2=cage_boxes)

    cage_mask_from_mask_rcnn: np.ndarray = ref_mask_rcnn_results['predictions']['masks'][cage_idxs[np.argmax(iou_matrix[0])]]
    
    # update the bounding boxes as well
    xc_tl, yc_tl, xc_br, yc_br = ref_mask_rcnn_results['predictions']['boxes'][cage_idxs[np.argmax(iou_matrix[0])]]
    
    cnts, _ = cv2.findContours(cage_mask_from_mask_rcnn.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    # contours of objects detected by Mask R-CNN
    mask_rcnn_obj_contours: List[np.ndarray] = [cnts[0]]
    
    caged_cell_instances_mask: np.ndarray = ref_mask_rcnn_results[
    'combined_obj_masks']['cell'][yc_tl: yc_br, xc_tl:xc_br] * cage_mask_from_mask_rcnn.astype(int)
    cell_ids_in_cage: np.ndarray = np.unique(caged_cell_instances_mask[caged_cell_instances_mask > 0])

    target_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(cage_mask_from_mask_rcnn.shape, np.uint8)
    # threshold to use for classifying the target cells for the reference Mask R-CNN scan
    threshold_for_pixel_classification_per_fl_channel: Dict[str, float] = thresholds_for_pixel_classification[ref_scan_id][lane_id]
    ...
    for cell_id in cell_ids_in_cage:
        ...
        if (np.sum(ref_out_images['Blue'][yc_tl:yc_br, xc_tl:xc_br] * this_cell_mask) >= 
            np.sum(this_cell_mask) * threshold_for_pixel_classification_per_fl_channel['Blue']):
            target_cells_mask_from_mask_rcnn = np.maximum(target_cells_mask_from_mask_rcnn, this_cell_mask)
            num_target_cells += 1
        # find the contour
        cnts, _ = cv2.findContours(this_cell_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    ref_img_with_mask_rcnn_masks = (255 * ref_img_with_mask_rcnn_masks.astype(float) / (2 ** 12 - 1)).astype(np.uint8)
    ref_img_with_mask_rcnn_masks = cv2.normalize(
        ref_img_with_mask_rcnn_masks, ref_img_with_mask_rcnn_masks, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX
    )
    ref_img_with_mask_rcnn_masks = ref_img_with_mask_rcnn_masks[yc_tl:yc_br, xc_tl:xc_br]
    ref_img_with_mask_rcnn_masks = cv2.cvtColor(ref_img_with_mask_rcnn_masks, cv2.COLOR_GRAY2RGB)

    # identify the target cells in the Mask R-CNN results with the Blue stain
    ref_img_with_mask_rcnn_masks = (ref_img_with_mask_rcnn_masks * 0.6 + 
                                    cv2.cvtColor(target_cells_mask_from_mask_rcnn, cv2.COLOR_GRAY2RGB) * colors['Blue'] * 0.4).astype(np.uint8)

   
    cv2.drawContours(ref_img_with_mask_rcnn_masks, mask_rcnn_obj_contours, -1, (255, 255, 0), 2)
    ...
    normalized_cropped_img: np.ndarray = (255 * out_images['White'][yc_tl: yc_br, xc_tl:xc_br].astype(float) / (2 ** 12 - 1)).astype(np.uint8)
    normalized_cropped_img = cv2.normalize(normalized_cropped_img, normalized_cropped_img, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    ...
    img_with_cell_masks = (img_with_cell_masks * 0.9 + 
                           cv2.cvtColor(foreground_cells_mask, cv2.COLOR_GRAY2RGB) * np.array([0, 255, 0]) * 0.1).astype(np.uint8)

    img_overlay: np.ndarray = normalized_cropped_img.copy()
    
    # threshold to use for classifying the pixels for the current scan
    threshold_for_pixel_classification_per_fl_channel: Dict[str, float] = thresholds_for_pixel_classification[scan_id][lane_id]
    
    for fl_channel_identifier in fl_channel_identifiers:
        if fl_channel_identifier not in threshold_for_pixel_classification_per_fl_channel:
            continue
        ...
        mask[img >= threshold_for_pixel_classification_per_fl_channel[fl_channel_identifier]] = 1
        mask = mask * foreground_cells_mask
        
        img_overlay = (img_overlay * 0.6 + 
                       cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB) * colors[fl_channel_identifier] * 0.4).astype(np.uint8)
...
def merge_cages_metrics_dataframes_using_cage_coords(
    ...
    key_0 = 'all'
    paired_idxs, unpaired_1, unpaired_2 = pair_cages_using_iou(cage_boxes=cages_metrics[key_0][
    ...
    print(f"Successfully paired {len(paired_idxs)} cages out of {len(cages_metrics[key_0])}!")
    ...
    for key in cages_metrics:
        ...
        ref_df: pd.DataFrame = ref_cages_metrics[key][columns_to_include_from_ref].copy()
        ...
        merged_cages_metrics[key] = df.merge(ref_df, on='cage_id', suffixes=('', '_ref'))

    return merged_cages_metrics


def gmm_fit(avg_fl_signal_per_cell: Dict[str, List[float]], fl_channel_identifier: str, show_plot: bool = True):
    ...
    try:
        threshold = brentq(lambda x: get_pdf(1, x) - get_pdf(0, x), min(data), max(data))
    except ValueError:
        threshold = min(data)
    ...
    if show_plot: 
        ...
        plt.hist(data, bins=100, density=True, alpha=0.5, color='skyblue', label=f"Log10(Mean {fl_channel_identifier} Signal) over Cells")
...
def kde_fit(avg_fl_signal_per_cell: Dict[str, List[float]], fl_channel_identifier: str, show_plot: bool = True):
...
def kmeans_fit(avg_fl_signal_per_cell: Dict[str, List[float]], fl_channel_identifier: str, show_plot: bool = True):
    data = np.array(avg_fl_signal_per_cell[fl_channel_identifier])
...
def gmm_fit_2(avg_fl_signal_per_cell: Dict[str, List[float]], fl_channel_identifier: str, show_plot: bool = True):
    ...
    while True:
        ...
        for i in range(len(means) - 1):
            normalized_distances.append((means[i + 1] - means[i]) / (np.sqrt(variances[i + 1]) + np.sqrt(variances[i])))

        normalized_distances = np.array(normalized_distances)
        if np.min(normalized_distances) < 2:
            i = np.argmin(normalized_distances)
            merged_mean = (weights[i] * means[i] + weights[i + 1] * means[i + 1]) / (weights[i] + weights[i + 1])
            merged_var = (weights[i] * (variances[i] + means[i] ** 2) + 
                          weights[i + 1] * (variances[i + 1] + means[i + 1] ** 2)) / (weights[i] + weights[i + 1]) - merged_mean ** 2
            means = np.array(means[:max(0, i)].tolist() + [merged_mean] + means[min(i + 2, len(means)):].tolist())
            variances = np.array(variances[:max(0, i)].tolist() + [merged_var] + variances[min(i + 2, len(variances)):].tolist())
            weights = np.array(weights[:max(0, i)].tolist() + [weights[i] + weights[i+1]] + weights[min(i + 2, len(weights)):].tolist())
...
#
###########################################################
def get_cages_metrics_old(bf_img_filename: str, 
                      raw_images_folder: str, 
                      mask_rcnn_results_folder: str, 
    ...
    for i, box in enumerate(preds['boxes']):
        ...
        if cell_area > 0:
            ...
            if np.sum(masked_blue) >= THRESHOLD_ON_AVG_BLUE_FOR_TARGET_CELLS * cell_area:
                # this is a target cell
                blue_pixel_values_over_target_cells += masked_blue[preds['masks'][i] > 0].flatten().astype(int).tolist()
            else:
                # this is an effector cell
                blue_pixel_values_over_effector_cells += masked_blue[preds['masks'][i] > 0].flatten().astype(int).tolist()
        else:
    ...
    for i, cage_semantic_mask in enumerate(cage_semantic_masks):
        ...
        caged_cell_instances_mask: np.ndarray = combined_obj_masks['cell'][ytl:ybr, xtl:xbr]
        cell_ids_in_cage: np.ndarray = np.unique(caged_cell_instances_mask[caged_cell_instances_mask > 0])
        ...
        # calculate the mask for all the objects, we do it this way to avoid double counting pixel in overlapping cell areas
        foreground_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
        target_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
        effector_cells_mask_from_mask_rcnn: np.ndarray = np.zeros(caged_cell_instances_mask.shape, np.uint8)
        ...
        for cell_id in cell_ids_in_cage:
            ...
            # take the union, we multiply by cage_mask_from_mask_rcnn to only consider parts inside the cage
            # we do not skip cells that are partially (even very small portion) inside the cage
            foreground_mask_from_mask_rcnn = np.maximum(foreground_mask_from_mask_rcnn, this_cell_mask * cage_mask_from_mask_rcnn)
            # check if this is a target cell using the average blue channel and THRESHOLD_ON_AVG_BLUE_FOR_TARGET_CELLS
            if np.sum(blue_img[ytl:ybr, xtl:xbr] * this_cell_mask) >= THRESHOLD_ON_AVG_BLUE_FOR_TARGET_CELLS * this_cell_mask.sum():
                num_targets_in_cage += 1
                target_cells_mask_from_mask_rcnn = np.maximum(target_cells_mask_from_mask_rcnn, 
                                                              this_cell_mask * cage_mask_from_mask_rcnn)
            else:
                num_effectors_in_cage += 1
                effector_cells_mask_from_mask_rcnn = np.maximum(effector_cells_mask_from_mask_rcnn, 
                                                                this_cell_mask * cage_mask_from_mask_rcnn)
        ...
        num_foreground_pixels_mask_rcnn[starting_cage_id + i] = num_foreground_pixels_in_cage
        num_target_pixels_mask_rcnn[starting_cage_id + i] = num_target_pixels_in_cage
        num_effector_pixels_mask_rcnn[starting_cage_id + i] = num_effector_pixels_in_cage
        ...
        # beads overlapping will cells (if detected properly) will be excluded below
        foreground_cell_mask[cage_semantic_mask == sem_segmentor_model.get_reverse_label_map()['cell']] = 1
        # only consider the semantic masks for cell inside the cage mask from Mask R-CNN
        ...
        # create a mask for pixels corresponding to target cells
        target_cell_pixels_mask: np.ndarray = np.zeros(cage_semantic_mask.shape, np.uint8)
        target_cell_pixels_mask[masked_blue >= THRESHOLD_ON_BLUE_FOR_TARGET_CELL_PIXELS] = 1
    
        # do the same for effector cells (to find dead effectors)
        effector_cell_pixels_mask: np.ndarray = np.zeros(cage_semantic_mask.shape, np.uint8)
        effector_cell_pixels_mask[masked_blue < THRESHOLD_ON_BLUE_FOR_TARGET_CELL_PIXELS] = 1
        # this step is needed to make sure 0/small blue value non-cell pixels are not classified as effector cell pixels
        ...
        total_red_signal_target_pixels[starting_cage_id + i] = int(np.sum(masked_red * target_cell_pixels_mask))
        total_red_signal_effector_pixels[starting_cage_id + i] = int(np.sum(masked_red * effector_cell_pixels_mask))
...
def generate_cage_plots(bf_img_filename: str, 
                        raw_images_folder: str, 
                        mask_rcnn_results_folder: str, 
                        inst_segmentor_model,
                        sem_segmentor_model):
    ...
    fl_img_filenames: Tuple[str] = get_fl_channel_filenames(bf_img_filename, {'Red': RED_OFFSET_TO_BF, 'Blue': BLUE_OFFSET_TO_BF})
    
    red_img_filename: str = fl_img_filenames[0]
    blue_img_filename: str = fl_img_filenames[1]

    # load BF, Blue and Red images
    bf_img: np.ndarray = cv2.imread(os.path.join(raw_images_folder, bf_img_filename), cv2.IMREAD_UNCHANGED)
    red_img: np.ndarray = cv2.imread(os.path.join(raw_images_folder, red_img_filename), cv2.IMREAD_UNCHANGED)
    blue_img: np.ndarray = cv2.imread(os.path.join(raw_images_folder, blue_img_filename), cv2.IMREAD_UNCHANGED)

    # Plot Mask R-CNN results
    normalized_bf_img: np.ndarray = (bf_img.astype(float) / (2**12 - 1) * 255).astype(np.uint8)
    normalized_bf_img: np.ndarray = cv2.normalize(normalized_bf_img, normalized_bf_img, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    debug_img: np.ndarray = show_detections(normalized_bf_img, preds, inst_segmentor_model.get_label_map())
    ...
    for i, cage_semantic_mask in enumerate(cage_semantic_masks):
        ...
        for cell_id in cell_ids_in_cage:
            ...
            # check if this is a target cell using the average blue channel and THRESHOLD_ON_AVG_BLUE_FOR_TARGET_CELLS
            if np.sum(blue_img[ytl:ybr, xtl:xbr] * this_cell_mask) >= THRESHOLD_ON_AVG_BLUE_FOR_TARGET_CELLS * this_cell_mask.sum():
                target_cells_mask_from_mask_rcnn = np.maximum(target_cells_mask_from_mask_rcnn, this_cell_mask * cage_mask_from_mask_rcnn)
                
                debug_img[ytl:ybr, xtl:xbr][this_cell_mask * cage_mask_from_mask_rcnn > 0] = (
                    debug_img[ytl:ybr, xtl:xbr][this_cell_mask * cage_mask_from_mask_rcnn > 0] * 0.6 +
                    (cv2.cvtColor((this_cell_mask * cage_mask_from_mask_rcnn), cv2.COLOR_GRAY2RGB) * np.array([255, 0, 0])
                    )[this_cell_mask * cage_mask_from_mask_rcnn > 0] * 0.4
                ).astype(np.uint8)
                
            else:
                effector_cells_mask_from_mask_rcnn = np.maximum(effector_cells_mask_from_mask_rcnn, this_cell_mask * cage_mask_from_mask_rcnn)

                debug_img[ytl:ybr, xtl:xbr][this_cell_mask * cage_mask_from_mask_rcnn > 0] = (
                    debug_img[ytl:ybr, xtl:xbr][this_cell_mask * cage_mask_from_mask_rcnn > 0] * 0.6 +
                    (cv2.cvtColor((this_cell_mask * cage_mask_from_mask_rcnn), cv2.COLOR_GRAY2RGB) * np.array([0, 255, 0])
                    )[this_cell_mask * cage_mask_from_mask_rcnn > 0] * 0.4
                ).astype(np.uint8)
...
def create_image_grid(images, grid_size, image_size):
    ...
    for idx, image in enumerate(images):
        ...
        grid_image[y_start:y_start+image_size[0], x_start:x_start+image_size[1]] = resized_image
