...
def generate_scatter_plots(features, labels, output_dir, prefix):
    ...
    for n_neighbors in [5, 15, 30, 50]:
        ...
        umap_2d = umap.UMAP(n_components=2, n_neighbors=n_neighbors, random_state=42).fit_transform(features)
        plt.figure(figsize=(14, 10), dpi=300)
        sns.scatterplot(x=umap_2d[:, 0], y=umap_2d[:, 1], hue=labels, style=labels, palette=palette, markers=markers_dict, s=25, alpha=0.8, edgecolor="none")
        plt.title(f"UMAP (n_neighbors={n_neighbors})", fontsize=20)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10, borderaxespad=0.)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"scatter_umap_nn{n_neighbors}.png"), bbox_inches='tight')
...
def process_level(level_name, embs_dict, out_dir, model_name, class_name):
    ...
    for i in range(num_entities):
        for j in range(num_entities):
            ...
            cov_P, cov_Q = full_covs[name_P], full_covs[name_Q]
            n_P, n_Q = embs_P.shape[0], embs_Q.shape[0]
            
            if i < j: # Compute symmetric metrics only once per pair
                metrics_sym['euclidean'][i, j] = metrics_sym['euclidean'][j, i] = dist_euclidean(mu_P, mu_Q)
                metrics_sym['cosine'][i, j] = metrics_sym['cosine'][j, i] = dist_cosine(mu_P, mu_Q)
                metrics_sym['wasserstein_diag'][i, j] = metrics_sym['wasserstein_diag'][j, i] = dist_wasserstein_diag(mu_P, mu_Q, var_P, var_Q)
                metrics_sym['sym_kl_diag'][i, j] = metrics_sym['sym_kl_diag'][j, i] = dist_sym_kl_diag(mu_P, mu_Q, var_P, var_Q)
                metrics_sym['bhattacharyya_diag'][i, j] = metrics_sym['bhattacharyya_diag'][j, i] = dist_bhattacharyya_diag(mu_P, mu_Q, var_P, var_Q)
                metrics_sym['hellinger_diag'][i, j] = metrics_sym['hellinger_diag'][j, i] = dist_hellinger_diag(mu_P, mu_Q, var_P, var_Q)
                metrics_sym['bhattacharyya_full'][i, j] = metrics_sym['bhattacharyya_full'][j, i] = dist_bhattacharyya_full(mu_P, mu_Q, cov_P, cov_Q)
                metrics_sym['mahalanobis_pooled'][i, j] = metrics_sym['mahalanobis_pooled'][j, i] = dist_mahalanobis_pooled(mu_P, mu_Q, cov_P, cov_Q, n_P, n_Q)
                metrics_sym['sliced_wasserstein'][i, j] = metrics_sym['sliced_wasserstein'][j, i] = dist_swd(embs_P, embs_Q)
