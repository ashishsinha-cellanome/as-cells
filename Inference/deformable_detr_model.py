...
class DeformableDetrObjectDetector(VisionModel):
    ...
    def load(self) -> None:
        ...
        self._hg_preprocessor = DeformableDetrImageProcessor.from_pretrained("SenseTime/deformable-detr")
        # modify the resizing part as we will replace the backbone with that of SAM (both sides will be equal for SAM)
        self._hg_preprocessor.size = {'longest_edge': max(self._model_input_size), 'shortest_edge': min(self._model_input_size)}
        ...
        self._metadata = {
            ...
            'model_extra_info': (
                'With SAM backbone' if self._backbone.lower() == 'sam' else 
