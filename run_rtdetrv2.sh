...
NUM_QUERIES=("300" "600")
FPN_TYPE=("fused" "simple" "tiny")
FPN_LAYERS=("[3,7,11]" "[4,8,12]")
CONFIGS=()
    ...
    for sched in "${SCHEDULERS[@]}"; do
        for backbone in "${BACKBONES[@]}"; do
            for data_path in "${DATA_CONFIG[@]}"; do
                for fpn_layers in "${FPN_LAYERS[@]}"; do
                  for num_queries in "${NUM_QUERIES[@]}"; do
                      for fpn_type in "${FPN_TYPE[@]}"; do
                            CONFIGS+=("optimizer.optimizer.lr=${lr} scheduler=${sched} model/backbone=${backbone} data=${data_path} model.rtdetr.num_queries=${num_queries} model.backbone.fpn_type=${fpn_type} model.backbone.output_indices_for_fpn=${fpn_layers}")
                        done
                    done
                done
            done
...
srun uv run train_rt_detr_v2.py \
  ...
  ${MY_CONFIG}
  # model.backbone.train_backbone=True \ # needed only for resnet backbone
