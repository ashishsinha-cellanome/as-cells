...
def build_train_plus_val_promoted(
    ...
    for image in dropped_val_images:
        ...
        new_image["file_name"] = f"../{val_split_name}/{image['file_name']}"
    ...
    out_payload["images"] = list(train_images) + promoted_images
    ...
    *,
    ...
    for image in train_plus_payload.get("images", []):
        ...
        if image_id not in promoted_id_set and file_name.startswith("../"):
            unexpected_relative_paths.append(file_name)
...
def main(cfg: DictConfig) -> None:
    ...
    if validation_results["unexpected_relative_paths"]:
        raise RuntimeError(
            "Found non-promoted images with relative path prefixes: "
            f"{validation_results['unexpected_relative_paths'][:10]}"
        )
