...
def create_split_annotations(
    original_annotations, split_image_ids, new_prefix, output_file
):
    print(f"Creating split annotations for {output_file}...")
    new_images = [
        img for img in original_annotations["images"] if img["id"] in split_image_ids
    ]
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Split and update dataset annotations."
    )
    parser.add_argument(
        "--base_dir",
        type=str,
...
def main():
    ...
    original_train_ann_file = os.path.join(
        base_dir, "train", "zhiyuan_objv2_train.json"
    )
    original_val_ann_file = os.path.join(base_dir, "val", "zhiyuan_objv2_val.json")

    new_val_ann_file = os.path.join(
        base_dir, "val", f"{output_suffix}_zhiyuan_objv2_val.json"
    )
    ...
    print(
        f"Number of images from validation to add to training: {len(new_train_images)}"
    )
    ...
    new_train_annotations = [
        ...
        if ann["image_id"] in new_train_image_ids
    ]
    print(
        f"Number of annotations from validation to add to training: {len(new_train_annotations)}"
    )
