...
# Identify the in-domain dataset(s)
in_domain_names = set(df_all.loc[df_all.split_type == "train_ds", "dataset"])

# Drop the duplicate reporting of that same dataset appearing again under test_ds
mask_dup = (df_all.split_type == "test_ds") & (df_all.dataset.isin(in_domain_names))
df_all = df_all[~mask_dup].copy()

df_all["domain"] = np.where(df_all.split_type == "train_ds",
                             "In-domain (train_ds test split)",
                             "Zero-shot (unseen test_ds)")
...
colors = [IN_DOMAIN_COLOR if v == "In-domain (train_ds test split)" else ZERO_SHOT_COLOR for v in d["domain"]]
...
legend_handles = [mpatches.Patch(color=IN_DOMAIN_COLOR, label="In-domain (train_ds test split)"),
                  mpatches.Patch(color=ZERO_SHOT_COLOR, label="Zero-shot (unseen test_ds)")]
...
for dom, color, marker in [("In-domain (train_ds test split)", IN_DOMAIN_COLOR, "s"),
                            ("Zero-shot (unseen test_ds)", ZERO_SHOT_COLOR, "o")]:
...
df_cls = df_cls[~((df_cls.split_type == "test_ds") & (df_cls.dataset.isin(in_domain_names)))]
