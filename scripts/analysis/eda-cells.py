...
from utils.cv_utils import show_detections
...
def plot_cell_size_distribution(df, save_name, log=False):
    ...
    if log:
        ax.set_yscale("log")
    sns.violinplot(
        x="label", y="mean_mask_area", data=df, hue="dataset", ax=ax, split=False
    )
...
if __name__ == "__main__":
    ...
    for dataset_path in tqdm(all_datasets, desc="Datasets"):
        ...
        image_stats_df = (
            cell_df.groupby(["dataset", "img_name", "label"])
            .agg(
        ...
        save_dset_plot_name = OUTPUT_DIR / (
            dataset_path.name + "_mask-size-distro-per-dataset.png"
        )
        ...
        print(
            f"[INFO] Unique labels present in the image: {sample['annotations'].label.unique().tolist()}"
        )
        ...
        gt_preds = {"masks": masks, "labels": labels, "boxes": boxes}
        # import pdb; pdb.set_trace()
        img_w_gt_boxes = show_detections(
            input_image=image,
            predictions=gt_preds,
            label_map=CLASS_IDS_TO_CLASS_NAMES_MAP,
        )
