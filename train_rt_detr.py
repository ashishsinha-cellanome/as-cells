...
def merge_configs(base_config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
...
def create_initial_checkpoint(config: Dict[str, Any]) -> str:
    ...
    if not os.path.exists(rtdetr_checkpoint_path):
        ...
        print("Pretrained models encoder input dims: ", [pretrained_rt_detr.model.encoder_input_proj[i][0].weight.shape[1] 
                                    for i in range(len(pretrained_rt_detr.model.encoder_input_proj))])
        print("Pretrained config encoder input dims: ", pretrained_rt_detr.model.backbone.intermediate_channel_sizes)
        ...
        print(f"DINOv2 backbone: {dinov2_backbone_config.dinov2_pretrained_backbone_name_or_path}")
        ...
        # breakpoint()
        # TODO: override the loss components here
        # pretrained_model_config_dict['auxiliary_loss']  = model_config['auxiliary_loss']
...
def setup_model(config: Dict[str, Any]) -> RTDETRLightningModule:
    ...
    # TODO: override the models config with ablation parameters
    # either here or during model's initialization
    model.config.auxiliary_loss = model_config['auxiliary_loss']
    model.config.use_focal_loss = model_config['use_focal_loss']
    model.config.weight_loss_vfl = model_config['weight_loss_vfl']
    # breakpoint()
    ...
    # Load test COCO Gt
    test_annot_path = os.path.join(config['dataset']['path'], 'images', 'test')
    test_json_path = os.path.join(config['dataset']['path'], 'test_annotations.json')
    ...
    # for debugging
    # test_coco_gt = val_coco_dataset.coco
    ...
    lightning_model = RTDETRLightningModule(
        ...
        test_coco_gt= val_coco_gt if config['debug'] else test_coco_gt, # TODO: for debugging change to val_coco_gt
        config = config,
    )
...
def setup_profiler(config: Dict[str, Any]):
    trainer_config = config['training']
    dir_name = os.path.join(config['checkpointing']['save_dir'], 'profiler_logs')
    # This schedule skips the first 5 batches (warmup),
    # then actively records the next 10 batches.
    # my_schedule = schedule(skip_first=5, wait=0, warmup=0, active=10, repeat=1)
    if trainer_config['profiler'] == 'simple':
        profiler = SimpleProfiler(
            dirpath=dir_name,  # Directory to save traces
            filename="rtdetr_profile", # Name of the trace file
            # schedule=my_schedule,
            # This handler auto-formats the output for TensorBoard
...
def int_or_str(value):
    """Tries to convert value to int, otherwise returns it as str."""
    try:
        return int(value)
    except ValueError:
        return str(value)
...
def main():
    ...
    default_run_name = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    parser.add_argument('--batch_size', '-bs',  type=int, help='Training batch size')
    parser.add_argument('--learning_rate', '-lr', type=float, help='Learning rate')
    parser.add_argument('--num_epochs', '-e', type=int, help='Number of training epochs')
    parser.add_argument('--num_workers', type=int, help='Number of data loading workers')
    parser.add_argument('--precision','-p', type=int_or_str, choices=[32, 16, 8, '16-mixed', '16-true'], default='16-mixed', help='precision')
    ...
    parser.add_argument('--wandb_name', type=str, default=default_run_name, help='WandB run name')
    ...
    parser.add_argument(
        '--debug',
        action='store_true',
        help='run in debug mode on val set and less batches'
    )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=callbacks,
        logger=logger,
        overfit_batches = trainer_config['num_overfit_samples'] if args.debug else 0,
        limit_test_batches = trainer_config['limit_test_batches'] if args.debug else 1.0,
        limit_train_batches = trainer_config['limit_train_batches'] if args.debug else 1.,
        limit_val_batches = trainer_config['limit_val_batches'] if args.debug else 1.,
        # fast_dev_run = trainer_config['fast_dev_run'],
        profiler = None if args.debug else profiler,
        
    )
    # Start training
    ckpt_path = config['initialization'].get('load_from_checkpoint')

    #TODO: fro debugging, remove laer
    if args.debug:
        print("\n" + "="*80)
        print("Manually calling data_module.setup() for 'fit' and 'test' stages...")
        data_module.setup(stage='fit')  # This creates the train and val datasets
        data_module.setup(stage='test') # This creates the test dataset
        print("[DEBUG] Data module setup complete.")
        print("="*80 + "\n")
    ...
    elif not args.test and not args.debug:
        # breakpoint()
        ckpt = torch.load(ckpt_path, weights_only=False, map_location='cpu')['callbacks']["ModelCheckpoint{'monitor': 'val_map', 'mode': 'max', 'every_n_train_steps': 0, 'every_n_epochs': 1, 'train_time_interval': None}"]
        print(f"Best model: {ckpt_path}")
        print(f"Best val_map: {ckpt['best_model_score']:.4f}")
        # Run test evaluation
        print("\nRunning test evaluation...")
        trainer.test(
            model, 
