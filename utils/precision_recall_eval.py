...
class AnnotationFilter:
    def __init__(
        ...
        classnames_to_exclude: List[str] = [
            name
            for name, mapped_name in self.classnames_mapping_dict.items()
            if mapped_name == "bg"
        ]
        self.class_ids_to_exclude: List[int] = [
            reverse_label_map[name] for name in classnames_to_exclude
        ]
        self.class_ids_mapping_dict = {
            reverse_label_map[name]: reverse_label_map[mapped_name]
    ...
    def apply(self, annots: dict) -> dict:
        ...
        annotations_df["label"] = annots["annotations"]["label"].apply(
            lambda x: class_ids_mapping_dict[x]
        )
        annotations_df = annotations_df[
            annotations_df["label"].apply(
                lambda x: False if x in self.class_ids_to_exclude else True
            )
        ]
        ...
        masks: List[np.ndarray] = [
            mask.copy()
            for idx, mask in enumerate(annots["masks"])
            if idx in idxs_to_keep
        ]
        return {
            "name": annots["name"],
            "image": annots["image"].copy(),
...
def evaluate_pr_per_image(
    ...
    for i in tqdm(range(len(predictions))):
        ...
        labels = (
            ...
            if "labels" in preds and len(preds["labels"]) > 0
            else np.zeros((0,))
        )
        ...
        for class_id in class_ids_of_interest:
            ...
            gt_boxes = gt_annotations.loc[
                gt_idxs, ["xtl", "ytl", "xbr", "ybr"]
            ].values.astype(int)
        ...
        for cid in class_ids_of_interest:
            tp, fp, fn = (
                image_metrics["tp"][cid],
                image_metrics["fp"][cid],
                image_metrics["fn"][cid],
            )
    ...
    for class_id in class_ids_of_interest:
        ...
        agg_f1[class_id] = (
            2
            * (agg_precision[class_id] * agg_recall[class_id])
            / (agg_precision[class_id] + agg_recall[class_id] + 1e-30)
        )

    # Calculate micro-average metrics (across all classes)
    micro_precision = total_tp_all_classes / (
        total_tp_all_classes + total_fp_all_classes + 1e-30
    )
    micro_recall = total_tp_all_classes / (
        total_tp_all_classes + total_fn_all_classes + 1e-30
    )
    micro_f1 = (
        2 * (micro_precision * micro_recall) / (micro_precision + micro_recall + 1e-30)
    )
...
def evaluate_pr_updated(
    ...
    num_true_positives_per_class: Dict[int, int] = {
        c_id: 0 for c_id in class_ids_of_interest
    }
    num_false_positives_per_class: Dict[int, int] = {
        c_id: 0 for c_id in class_ids_of_interest
    }
    num_false_negatives_per_class: Dict[int, int] = {
        c_id: 0 for c_id in class_ids_of_interest
    }
    ...
    for idx in range(len(dataset)):
        ...
        preds = predictions[
            idx
        ].copy()  # Use .copy() to avoid modifying the original list
        ...
        if annotation_filter is not None:
            ...
            if remap_dict and "labels" in preds and len(preds["labels"]) > 0:
                ...
                new_labels = np.array(
                    [remap_dict.get(label, label) for label in original_labels]
                )
        ...
        for class_id in class_ids_of_interest:
            ...
            idxs = annots["annotations"][
                annots["annotations"]["label"] == class_id
            ].index.values

            gt_boxes = (
                annots["annotations"]
                .loc[idxs, ["xtl", "ytl", "xbr", "ybr"]]
                .values.astype(int)
            )
            ...
            if use_mask:
                ...
                det_masks = [
                    mask for i, mask in enumerate(masks) if labels[i] == class_id
                ]
                paired_idx, unpaired_gts, unpaired_dets = pair_gts_dets_mask(
                    gt_boxes, gt_masks, det_boxes, det_masks, min_iou
                )
            else:
    ...
    for class_id in class_ids_of_interest:
        ...
        precision_per_class[class_id] = num_true_positives_per_class[class_id] / (
            num_true_positives_per_class[class_id]
            + num_false_positives_per_class[class_id]
            + 1e-30
        )  # precision
        recall_per_class[class_id] = num_true_positives_per_class[class_id] / (
            num_true_positives_per_class[class_id]
            + num_false_negatives_per_class[class_id]
            + 1e-30
        )  # recall
    ...
    return (
        ...
        num_false_negatives,
        num_true_positives
        / (num_true_positives + num_false_positives + 1e-30),  # micro precision
        num_true_positives
        / (num_true_positives + num_false_negatives + 1e-30),  # micro recall
    )
...
def calculate_confusion_matrix(
    ...
    confusion_matrix: np.ndarray = np.zeros(
        (len(gt_class_ids) + 1, len(gt_class_ids) + 1), dtype=int
    )
    matrix_idx_to_class_id_map: Dict[int, int] = {
        i: gt_class_ids[i] for i in range(len(gt_class_ids))
    }
    class_id_to_matrix_idx_map: Dict[int, int] = {
        gt_class_ids[i]: i for i in range(len(gt_class_ids))
    }

    for idx in range(len(dataset)):
        annots = dataset[idx]
        confusion_matrix += calculate_sample_confusion_matrix(
            predictions[idx], annots, gt_class_ids, use_mask, min_iou
        )
...
def calculate_sample_confusion_matrix(
    ...
    gt_boxes = data_sample["annotations"][["xtl", "ytl", "xbr", "ybr"]].values.astype(
        int
    )
    ...
    if use_mask:
        ...
        paired_idx, unpaired_dets, unpaired_gts = pair_gts_dets_mask(
            det_boxes, det_masks, gt_boxes, gt_masks, min_iou
        )
    else:
        paired_idx, unpaired_dets, unpaired_gts = pair_gts_dets_bbox(
            det_boxes, gt_boxes, min_iou
        )

    for i in unpaired_gts:
        confusion_matrix[
            len(gt_class_ids), class_id_to_matrix_idx_map[gt_labels[i]]
        ] += 1

    for i in unpaired_dets:
        confusion_matrix[
            class_id_to_matrix_idx_map[det_labels[i]], len(gt_class_ids)
        ] += 1

    for i, j in paired_idx:
        confusion_matrix[
            class_id_to_matrix_idx_map[det_labels[i]],
            class_id_to_matrix_idx_map[gt_labels[j]],
        ] += 1
...
def p_r_based_on_c_m(
    ...
    c_m: np.ndarray = calculate_confusion_matrix(
        predictions, dataset, use_masks, min_iou
    )
    ...
    row_idxs_to_keep: List[int] = [
        i for i, c_id in enumerate(dataset_class_ids) if c_id in ids_predicted_by_model
    ]
    model_classnames_to_keep: List[str] = [
        class_ids_to_classnames_map[c_id]
        for c_id in dataset_class_ids
    ...
    c_m_df = pd.DataFrame(
        index=model_classnames_to_keep + ["FN"],
        columns=dataset_class_names + ["FP"],
        data=c_m,
    )  # the last column is FPs
        ...
        if annotation_filter is not None:
            ...
            for k, v in annotation_filter.class_ids_mapping_dict.items():
                if v == class_id:
                    ...
                    mapped_ids.append(
                        dataset.class_names_to_ids_map[class_ids_to_classnames_map[k]]
                    )
        mapped_ids = list(set(mapped_ids))
        model_class_id_to_dataset_classnames_map[class_id] = [
            class_ids_to_classnames_map[k] for k in mapped_ids
        ]
    ...
    for class_id in model_label_map:
        ...
        fp_col_names: List[str] = [
            col
            for col in c_m_df.columns.tolist()
            if col not in model_class_id_to_dataset_classnames_map[class_id]
        ]
        row = c_m_df.loc[
            class_ids_to_classnames_map[class_id],
            model_class_id_to_dataset_classnames_map[class_id] + fp_col_names,
        ].values
        tp = np.sum(row[: len(model_class_id_to_dataset_classnames_map[class_id])])
        if np.sum(row) > 0:
            precision[class_ids_to_classnames_map[class_id]] = np.round(
                tp / np.sum(row), 3
            )
        # recall
        fn_row_names: List[str] = [
            ro
            for ro in c_m_df.index.tolist()
            if ro != class_ids_to_classnames_map[class_id]
        ]
        fn = c_m_df.loc[
            fn_row_names, model_class_id_to_dataset_classnames_map[class_id]
        ].values.sum()
        if tp + fn > 0:
    ...
    for class_id in dataset_class_ids:
        if (
            annotation_filter is not None
            and class_id in annotation_filter.class_ids_mapping_dict
        ):
        ...
        if class_ids_to_classnames_map[model_class_id] not in c_m_df.index:
            ...
            continue
        tp = c_m_df.loc[
            class_ids_to_classnames_map[model_class_id],
            class_ids_to_classnames_map[class_id],
        ]
        fn_row_names: List[str] = [
            ro
            for ro in c_m_df.index.tolist()
            if ro != class_ids_to_classnames_map[model_class_id]
        ]
        fn = c_m_df.loc[fn_row_names, class_ids_to_classnames_map[class_id]].sum()
        if tp + fn > 0:
            recall_w_break_down[class_ids_to_classnames_map[class_id]] = np.round(
                tp / (tp + fn), 3
            )
        else:
    ...
    p_r_df = pd.DataFrame(
        data=[precision, recall], index=["Precision", "Recall"]
    ).transpose()
    p_r_with_break_down_df = pd.DataFrame(
        data=[precision, recall_w_break_down], index=["Precision", "Recall"]
    ).transpose()
