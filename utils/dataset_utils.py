    ...
    for t_config in transform_list:
        ...
        if hasattr(A, name):
            ...
            try:
                ...
                print(
                    f"[ERROR] Failed to initialize transform {name} with params {params}: {e}"
                )
        else:
...
def get_transform(
    ...
    train: bool = True,
    transforms_config: Optional[List[Dict]] = None,
    ...
    if transforms_config is not None:
        ...
        trsfms.extend(
            [
                ...
                A.PadIfNeeded(
                    min_height=model_input_height, min_width=model_input_width
                ),
...
def create_dataset_classes(
    ...
    annotations_images_map: pd.DataFrame = pd.read_csv(
        os.path.join(dataset_path, "annotation_images_mapping.csv")
    )
    ...
    image_columns: List[str] = [
        column_name for column_name in columns if "white_dz" in column_name.lower()
    ]

    num_images_to_consider_for_each_annotation: int = (
    ...
    if len(image_columns) > 1:
        if only_use_best_focus_image:
            image_columns = [
                ...
                if "white_dz0" in column_name.lower()
            ]
            num_images_to_consider_for_each_annotation = min(
                1, num_images_to_consider_for_each_annotation
            )
    ...
    if len(image_columns) == 0:
        ...
        for column_name in columns:
            if (
                "bf" in column_name.lower()
                or "white" in column_name.lower()
                or "images" in column_name.lower()
            ):
...
def mask_rcnn_processor(
    ...
    for annots in annotations["annotations"]:
        ...
        masks.append(
            np.expand_dims(coco_mask_util.decode(annots["segmentation"]), axis=0)
        )
