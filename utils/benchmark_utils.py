...
import torch
from models.yolo_model import DEFAULT_CROP_CORNERS_10x, DEFAULT_RESIZE_10x, DEFAULT_CROP_CORNERS_4x, DEFAULT_RESIZE_4x
import json 
    ...
    try:
        ...
        with open(json_path, 'r') as f:
            ...
            if 'annotations' in data and isinstance(data['annotations'], list):
                ...
                print(f"Warning: 'annotations' key not found or is not a list in {json_path}")
...
def get_test_set(DATASET_PATHS, CLASS_NAME2ID_MAPPING):
    ...
    print(f"{len(DATASET_PATHS)} test datasets with a total of {len(test_dataset)} samples for evaluation.")
    return test_dataset
...
def infer(detector, test_dataset):
    ...
    for datasample in tqdm(test_dataset):
        if 'yolo' in detector.get_model_name().lower():
            detections, runtime = run_model(
                ...
                input_resize=DEFAULT_RESIZE_4x if detector.get_metadata()['magnification'] == '4x' else DEFAULT_RESIZE_10x, # need to pass them as the weights files does not provide them
                input_crop_corners=DEFAULT_CROP_CORNERS_4x if detector.get_metadata()['magnification'] == '4x' else DEFAULT_CROP_CORNERS_10x, # need to pass them as the weights files does not provide them
    ...
    print(f"Running the {detector.get_model_name()} model took {np.round(np.mean(runtimes) * 1000)}ms on average per image")
...
def show_detections_updated(
    ...
    for i in range(len(labels)):
        ...
        if 'masks' in detections and detections['masks'] is not None and len(detections['masks']) > i:
