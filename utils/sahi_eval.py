...
def run_sahi_sliced_eval(
    ...
    if slice_height is None:
        slice_height = input_size

    slice_width = sahi_config.get("slice_width")
    if slice_width is None:
        slice_width = input_size

    overlap_height_ratio = resolve_sahi_overlap(
        sahi_config, slice_height, is_height=True
    )
    overlap_width_ratio = resolve_sahi_overlap(
        sahi_config, slice_width, is_height=False
    )
    ...
    if export_dir is not None:
        ...
        result.export_visuals(
            export_dir=export_dir, file_name=file_name, text_size=0.5, rect_th=2
        )
