...
class YOLOv5DataModule(pl.LightningDataModule):
    ...
    def _convert_coco_to_yolo(self, split_name, image_dir, json_path):
        ...
        for img_id in tqdm(
            coco.getImgIds(), desc=f"Converting {split_name} COCO to YOLO format"
        ):
            ...
            for ann in anns:
                ...
                yolo_labels.append(
                    f"{yolo_class_id} {x_c:.6f} {y_c:.6f} {w_norm:.6f} {h_norm:.6f}"
                )
    ...
    def setup(self, stage=None):
        ...
        if self.trainer.is_global_zero and not self.yaml_path.exists():
            ...
            self._convert_coco_to_yolo(
                self.data_config.train_name,
                os.path.join(dp, "images", self.data_config.train_name),
                os.path.join(dp, f"{self.data_config.train_name}_annotations.json"),
            )
            self._convert_coco_to_yolo(
                self.data_config.val_name,
                os.path.join(dp, "images", self.data_config.val_name),
                os.path.join(dp, f"{self.data_config.val_name}_annotations.json"),
            )
            self._convert_coco_to_yolo(
                self.data_config.test_name,
                os.path.join(dp, "images", self.data_config.test_name),
                os.path.join(dp, f"{self.data_config.test_name}_annotations.json"),
            )
        ...
        yolo_dataloaders = _import_from_yolo_repo(
            self.yolo_repo_path, "utils.dataloaders"
        )
