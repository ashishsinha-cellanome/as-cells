...
class YOLOv5LightningModule(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if model_cfg.weights:
            weights_path = (
                model_cfg.weights
                if str(model_cfg.weights).endswith(".pt")
                else f"{model_cfg.weights}.pt"
            )
            if os.path.exists(weights_path):
                ...
                rank_zero_print(
                    f"[INFO] Successfully loaded pretrained weights from: {weights_path}"
                )
            else:
                rank_zero_print(
                    f"[INFO] Pretrained weights not found at: {weights_path}"
                )
    ...
    def _get_ema_model(self):
        ...
        for cb in self.trainer.callbacks:
            if type(cb).__name__ == "EMACallback" and hasattr(cb, "ema_model"):
                return (
                    cb.ema_model.module
                    if hasattr(cb.ema_model, "module")
                    else cb.ema_model
                )
        return None
    ...
    def _shared_eval_step(
        self, batch, batch_idx, model_instance, prefix="val", visualize=True
    ):
        ...
        for i, det in enumerate(preds):
            ...
            if len(det):
                det[:, :4] = self._scale_boxes(
                    imgs.shape[2:], det[:, :4], orig_shape
                ).round()
        ...
        if visualize:
            viz_counter = (
                self.val_viz_counter if prefix == "val" else self.test_viz_counter
            )
            if (
                self.current_epoch % self.config.checkpointing.visualize_every_n_epochs
                == 0
            ) and (
                viz_counter < self.config.checkpointing.visualize_samples
                or self.config.checkpointing.visualize_samples == -1
            ):
                ...
                new_counter = self._visualize_batch(
                    save_dir,
                    post_processed_outputs,
                    imgs,
                    paths,
                ...
                if prefix == "val":
                    self.val_viz_counter = new_counter
                else:
                    self.test_viz_counter = new_counter
    ...
    def validation_step(self, batch, batch_idx):
        ...
        res = self._shared_eval_step(
            batch, batch_idx, model_instance=self.model, prefix="val", visualize=True
        )
        ...
        if hasattr(self.config.model, "ema") and self.config.model.ema.enabled:
            ...
            if ema_model is not None:
                ...
                res_ema = self._shared_eval_step(
                    batch,
                    batch_idx,
                    model_instance=ema_model,
                    prefix="val",
                    visualize=False,
                )
    ...
    def test_step(self, batch, batch_idx):
        # Regular Model
        res = self._shared_eval_step(
            batch, batch_idx, model_instance=self.model, prefix="test", visualize=True
        )
    ...
    def on_validation_epoch_end(self):
        ...
        self._compute_and_log_metrics(
            self.validation_step_outputs, self.val_coco_gt, prefix="val", suffix=""
        )
        ...
        if (
            ...
            and len(self.validation_step_outputs_ema) > 0
        ):
            self._compute_and_log_metrics(
                self.validation_step_outputs_ema,
                self.val_coco_gt,
    ...
    def on_test_epoch_end(self):
        # Process regular outputs
        self._compute_and_log_metrics(
            self.test_step_outputs, self.test_coco_gt, prefix="test", suffix=""
        )
        self.test_step_outputs.clear()

        # Process EMA outputs
        if (
            hasattr(self, "test_step_outputs_ema")
            and len(self.test_step_outputs_ema) > 0
        ):
            self._compute_and_log_metrics(
                self.test_step_outputs_ema,
                self.test_coco_gt,
            ...
            if len(predictions) > 0 and coco_gt is not None:
                try:
                    ...
                    metrics = {
                        ...
                        "mar_1": round(stats[6], 4),
                        "mar_10": round(stats[7], 4),
                        f"mar_{self.config.model.max_detections}": round(stats[8], 4),
                    }

                    if (
                        hasattr(coco_evaluator, "eval")
                        and "precision" in coco_evaluator.eval
                    ):
                        precisions, recalls = (
                            coco_evaluator.eval["precision"],
                            coco_evaluator.eval["recall"],
                        )
                        ...
                        for ann in coco_gt.dataset.get("annotations", []):
                            if ann["image_id"] in coco_evaluator.params.imgIds:
                                ...
                                labels_per_class[c_id] = (
                                    labels_per_class.get(c_id, 0) + 1
                                )
                                total_labels += 1
                        ...
                        for i, catId in enumerate(coco_evaluator.params.catIds):
                            ...
                            if len(s[s > -1]) > 0:
                                metrics[f"map_{cat_name}"] = round(
                                    float(np.mean(s[s > -1])), 4
                                )
                        ...
                        avg_p = (
                            np.mean([row["P"] for row in table_rows])
                            if table_rows
                            else 0.0
                        )
                        avg_r = (
                            np.mean([row["R"] for row in table_rows])
                            if table_rows
                            else 0.0
                        )
                        ...
                        print(
                            f"\n{prefix.capitalize()} set performance"
                            + (" (EMA)" if "_ema" in suffix else "")
                        )
    ...
    def _visualize_batch(
        self, save_dir, post_processed_outputs, imgs, paths, shapes, counter, prefix
    ):
        ...
        for i in range(len(paths)):
            ...
            if coco_gt and img_id in coco_gt.imgs:
                ...
                image = self.draw_boxes(
                    image, gt_boxes, gt_labels, color_override=(0, 255, 0)
                )
            ...
            rank = (
                self.global_rank if dist.is_available() and dist.is_initialized() else 0
            )
    ...
    def configure_optimizers(self):
        ...
        if opt_cfg.type == "sgd":
            optimizer = torch.optim.SGD(
                g0, lr=opt_cfg.lr, momentum=opt_cfg.momentum, nesterov=opt_cfg.nesterov
            )
        else:
            optimizer = torch.optim.Adam(
                g0, lr=opt_cfg.lr, betas=(opt_cfg.momentum, 0.999)
            )
