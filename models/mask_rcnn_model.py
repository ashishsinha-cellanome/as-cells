...
class MaskRCNNInstanceSegmentation(VisionModel):
    def __init__(
        ...
        model_input_size: Optional[
            Tuple[int, int]
        ] = DEFAULT_MODEL_INPUT_SIZE,  # not included in the weights, not really needed for Mask R-CNN
        ...
        self._mask_threshold_for_binary_conversion = (
            mask_threshold_for_binary_conversion
        )
    ...
    def load(self):
        ...
        self._model = get_instance_segmentation_model(
            num_classes=len(self._label_map) + 1, anchor_sizes=self._anchor_sizes
        )
    ...
    def detect_batch(
        ...
        img_tensor_list: List[torch.tensor] = [
            F.to_tensor(im).unsqueeze(0).to(self._device) for im in input_images_list
        ]
    ...
    def postprocess(
        ...
        for sample_index in range(len(outputs)):
            ...
            for i in range(out[sample_index]["boxes"].shape[0]):
                ...
                if (
                    out[sample_index]["scores"][i] < self._confidence
                    or ytl >= ybr
                    or xtl >= xbr
                ):
                    continue
                if (
                    out[sample_index]["labels"][i]
                    in self._class_ids_to_exclude_from_dets
                ):
                    continue
                if self._mask_threshold_for_bbox_expansion > 0:
                    ...
                    pos = torch.where(
                        all_mask_tensors[i, 0]
                        >= self._mask_threshold_for_bbox_expansion
                    )
                    ...
                    if 0 < (xmax - xmin) <= (1 + MAX_BBOX_EXPANSION_FACTOR) * (
                        ...
                        delta_x: int = int(
                            DEFAULT_BBOX_EXPANSION_FACTOR * (xbr - xtl) / 2
                        )
                        delta_y: int = int(
                            DEFAULT_BBOX_EXPANSION_FACTOR * (ybr - ytl) / 2
                        )
                ...
                prob_mask_this_cell = to_numpy(
                    all_mask_tensors[i, 0, ytl:ybr, xtl:xbr]
                ).astype(float)
                # threshold the probability mask to get a binary one
                mask_this_cell: np.ndarray = np.zeros(
                    prob_mask_this_cell.shape, dtype=np.uint8
                )
                mask_this_cell[
                    prob_mask_this_cell >= self._mask_threshold_for_binary_conversion
                ] = 1
