    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        if isinstance(self.backbone_config, Dinov2BackBoneWithFPNConfig):
            ...
            if self._name_or_path:
                ...
                self.backbone_config = RTDetrV2Config.from_pretrained(
                    self._name_or_path
                ).backbone_config
            else:
...
class RTDetrV2ForObjectDetectionWithCustomBackbone(RTDetrV2ForObjectDetection):
    ...
    def from_pretrained(
        ...
        if config is None:
            config = RTDetrV2ConfigWithCustomBackBone.from_pretrained(
    ...
    def __init__(self, config):
        ...
        if isinstance(config.backbone_config, Dinov2BackBoneWithFPNConfig):
            print(
                "[INFO]: The passed config.backbone_config is a custom one of type Dinov2BackBoneWithFPNConfig. "
            ...
            if config._name_or_path:
                # set it the default one used by the pre-configured model
                print(
                    f"[INFO]: The RT-DETRv2 model is instantiated with the backbone_config taken from {config._name_or_path}. "
                    f"This backbone will be replaced with the custom one once the RT-DETR model is instantiated."
                )
                config.backbone_config = RTDetrV2Config.from_pretrained(
                ...
                print(
                    "[INFO]: The RT-DETRv2 model is instantiated with the default backbone config! "
                ...
                if (
                    len(config.backbone_config.hidden_sizes[1:])
                    != len(num_intermediate_channels)
                    or len(
                        [
                            s
                            for s in config.backbone_config.hidden_sizes[1:]
                            if s not in num_intermediate_channels
                        ]
                    )
                    > 0
                ):
                    print(
                        f"[ERROR]: Incorrect model architecture! The number of intermediate channels {config.backbone_config.hidden_sizes[1:]} "
