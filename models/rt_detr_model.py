...
DEFAULT_MODEL_INPUT_SIZE: Final[int] = (
    640,
    640,
)  # for the RT-DETR model with the default Resnet50 backbone
...
DINOV2_MODEL_INPUT_SIZE: Final[int] = (
    672,
    672,
)  # for the RT-DETR model with DINOv2 backbone
    ...
    def load(self) -> None:
        ...
        if self._backbone.lower() == "default":
            ...
            dinov2_backbone = Dinov2BackBoneWithFPN.from_pretrained(
                "facebook/dinov2-base", output_indices_for_fpn=[2, 7, 12]
            )
            ...
            rt_detr_model_with_dinov2_backbone_config = (
                RTDetrV2ConfigWithCustomBackBone(**pretrained_model_config_dict)
            )
            # replace the backbone_config with that of DINOv2 with backbone
            rt_detr_model_with_dinov2_backbone_config.backbone_config = (
                dinov2_backbone_config
            )

            # build the model with random weights from the config
            self._model = RTDetrV2ForObjectDetectionWithCustomBackbone(
                rt_detr_model_with_dinov2_backbone_config
            )
        ...
        self._metadata = {
            ...
            "model_extra_info": (
                "RT-DETRv1 With Resnet50 backbone"
                if self._backbone.lower() == "default"
                else (
                    "RT-DETRv2 With DINOv2 backbone"
                    if self._backbone.lower() == "dinov2"
                    else ""
                )
            ),
