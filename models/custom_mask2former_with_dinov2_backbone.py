...
from dinov2_backbone_with_fpn_2 import Dinov2BackBoneWithFPNConfig, Dinov2BackBoneWithFPN
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        if isinstance(self.backbone_config, Dinov2BackBoneWithFPNConfig):
            ...
            if self._name_or_path:
                ...
                self.backbone_config = Mask2FormerConfig.from_pretrained(self._name_or_path).backbone_config
            else:
...
class Mask2FormerSegmentationWithCustomBackbone(Mask2FormerForUniversalSegmentation):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, config=None, **kwargs):
        ...
        if config is None:
            config = Mask2FormerConfigWithCustomBackBone.from_pretrained(pretrained_model_name_or_path, **kwargs)
    ...
    def __init__(self, config):
        if isinstance(config.backbone_config, Dinov2BackBoneWithFPNConfig):
            ...
            if config._name_or_path:
                # set it the default one used by the pre-configured model
                print(f"[INFO]: The Mask2Former model is instantiated with the backbone_config taken from {config._name_or_path}. "
                      f"This backbone will be replaced with the custom one once the Mask2Former model is instantiated.")
                config.backbone_config = Mask2FormerConfig.from_pretrained(config._name_or_path).backbone_config
            else:
                # this part may lead to incorrectly built Mask2Former model; this is because the feature map resolutions of the 
                # backbone
                print(f"[INFO]: The Mask2Former model is instantiated with the default backbone config!"
                      f"This backbone will be replaced with the custom one once the Mask2Former model is instantiated.")
            ...
            self.model.pixel_level_module.encoder = Dinov2BackBoneWithFPN(backbone_config)
